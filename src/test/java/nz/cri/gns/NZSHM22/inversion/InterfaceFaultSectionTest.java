...
public class InterfaceFaultSectionTest {
    ...
    private SectionClusterList getSectionClusterList(
            List<FaultSection> subSections,
            Map<IDPairing, Double> subSectionDistances,
            int minSectionsInRupture) {
        /*
         * test help function
         */
        Map<IDPairing, Double> sectionAzimuths =
                Maps.newHashMap(); // just an empty list, since we're not using azimuths now
        ...
        // jumps
        laughTest.setMaxAzimuthChange(
                Double.MAX_VALUE); // azimuth change constraints makes no sense with 2 axes
    ...
    public void testRuptureGeneratorSetup() throws IOException {
        ...
        U3FaultSystemIO.fsDataToXML(
                doc0.getRootElement(), FaultModels.XML_ELEMENT_NAME, null, null, subSections);
        ...
        assertEquals(
                1, getRuptures(getSectionClusterList(subSections, subSectionDistances, 9)).size());
        assertEquals(
                3, getRuptures(getSectionClusterList(subSections, subSectionDistances, 8)).size());
        assertEquals(
                6, getRuptures(getSectionClusterList(subSections, subSectionDistances, 7)).size());
        assertEquals(
                10, getRuptures(getSectionClusterList(subSections, subSectionDistances, 6)).size());
        assertEquals(
                15, getRuptures(getSectionClusterList(subSections, subSectionDistances, 5)).size());

        SectionClusterList clusters = getSectionClusterList(subSections, subSectionDistances, 6);
        List<List<Integer>> ruptures =
                getRuptures(getSectionClusterList(subSections, subSectionDistances, 6));
