...
public class MFDPlotCalc {
    ...
    public static SummedMagFreqDist calcNucleationMFD_forParentSect(FaultSystemSolution solution, Set<Integer> parentSectionIDs, double minMag, double maxMag, int numMag) {
        ...
        for (int sectIndex = 0; sectIndex < rupSet.getNumSections(); sectIndex++) {
            if (parentSectionIDs.contains(rupSet.getFaultSectionData(sectIndex).getParentSectionId())) {
                IncrementalMagFreqDist subMFD = calcNucleationMFD_forSect(solution, sectIndex, minMag, maxMag, numMag);
     ...
     */
    public  static IncrementalMagFreqDist calcNucleationMFD_forSect( FaultSystemSolution solution, int sectIndex, double minMag, double maxMag, int numMag) {
        FaultSystemRupSet rupSet = solution.getRupSet();
        ...
        if (rups != null) {
            for (int r : rups) {
                ...
                if (rupMagDist == null)
                    mfd.addResampledMagRate(rupSet.getMagForRup(r), solution.getRateForRup(r)*nucleationScalar, true);
                else
    ...
    public static IncrementalMagFreqDist calcParticipationMFD_forParentSect(FaultSystemSolution solution, Set<Integer> parentSectionIDs, double minMag, double maxMag, int numMag) {
        ArbIncrementalMagFreqDist mfd = new ArbIncrementalMagFreqDist(minMag, maxMag, numMag);
        FaultSystemRupSet rupSet = solution.getRupSet();
...
//            System.out.println(rupSet);
    ...
    public static SummedMagFreqDist getFinalSubSeismoOnFaultMFDForSects(FaultSystemSolution solution, Set<Integer> parentSectionIDs) {
