 ...
 * ruptures.
 *
 * <p>Can build parallel joint ruptures , i.e. joint ruptures with splays where the subduction and
 * crustal ruptures run in parallel.
 */
public class JointRuptureBuilderParallel {
    ...
    public static Jump findJump(
        ...
        for (FaultSection section : targetCluster.subSects) {
            ...
            if (distance <= maxJumpDist) {
                jumps.add(
                        new Jump(
                                subductionSections.get(nearest),
                                subductionRupture.clusters[0],
                                section,
                                targetCluster,
                                distance));
            }
        ...
        if (actualCoverage >= coverage) {
            ...
            // targetCluster.addConnection(new Jump(jump.toSection, jump.toCluster,
            // jump.fromSection, jump.fromCluster, jump.distance));
            return jump;
        }
    ...
    public boolean isCrustal(ClusterRupture rupture) {
        return Arrays.stream(rupture.clusters)
                .noneMatch(c -> c instanceof DownDipFaultSubSectionCluster);
    }
    ...
    /**
     * Finds a jump between the two ruptures if enough sections of the crustalRupture are within
     * maxJumpDist
     *
     ...
     * @return
     */
    public static JointJump findJump(
            ClusterRupture subductionRupture,
            ...
            for (FaultSection section : cluster.subSects) {
                ...
                for (int ss = 0; ss < subductionSections.size(); ss++) {
                    double candidateDist =
                            distAzCalc.getDistance(subductionSections.get(ss), section);
                    if (candidateDist < distance) {
                        nearest = ss;
                        distance = candidateDist;
                    }
    ...
    public static List<Jump> findAllJumps(
    ...
    static ClusterRupture grow(
            ClusterRupture main,
            Set<FaultSubsectionCluster> connectedClusters,
            FaultSubsectionCluster fromCluster) {
        Optional<Jump> optionalJump =
                fromCluster.getConnections().stream()
    ...
    static ClusterRupture growSplay(
            ClusterRupture nucleation, Set<FaultSubsectionCluster> connectedClusters, Jump jump) {
        ClusterRupture main = nucleation.take(jump);
        return grow(main, connectedClusters, jump.toCluster);
    }

    public static List<ClusterRupture> makeSplays(
            ClusterRupture nucleation,
            List<FaultSubsectionCluster> targetClusters,
            SectionDistanceAzimuthCalculator distAzCalc) {
        ...
        Set<FaultSubsectionCluster> connectedClusters =
                jumps.stream().map(j -> j.toCluster).collect(Collectors.toSet());
    ...
    public JointRuptureBuilderParallel(
            List<ClusterRupture> ruptures, SectionDistanceAzimuthCalculator distAzCalc) {
    ...
    public ClusterRupture makeRuptureSplay(
            ClusterRupture nucleation, double maxJumpDist, int maxSplayCount) {
        ...
        for (ClusterRupture crustalRupture : crustalRuptures) {
            List<Integer> sectionIds =
                    crustalRupture.buildOrderedSectionList().stream()
    ...
    //            DownDipFaultSubSectionCluster cluster = (DownDipFaultSubSectionCluster)
    // subductionRupture.clusters[0];
    ...
    ////                    if(target.getTotalNumSects() > (subductionRupture.getTotalNumSects() *
    // 5)) {
    ...
    //                                    return null;
    //                                }
    //                                return ManipulatedClusterRupture.join(jump.rupture,
    // subductionRupture, distAzCalc);
    ...
    //                    .filter(r -> r.clusters[0].subSects.size() >= subductionMinCount &&
    // r.clusters[0].subSects.size() <= subductionMaxCount)
    //                    .filter(r -> jumps.containsKey(first(r).getSectionId()) ||
    // jumps.containsKey(last(r).getSectionId()))
    //                    .collect(Collectors.toList());
    //
    //
    //            System.out.println("Generating joint ruptures from " + subductionRuptures.size() +
    // " subduction ruptures and " + jumps.values.size() + " jump targets for parent ID " +
    // parentId);
    //            List<ClusterRupture> jointRuptures = new ArrayList<>();
