...
public class SectionPolygons {
     ...
     */
    static SectionPolygons create(
            List<? extends FaultSection> srcList, Double buf, Double len, double minBufferSize) {
    ...
    private static void initFM(
            SectionPolygons fp, List<? extends FaultSection> faults, double len) {
    ...
    private void applyBuffer(double buf, double minBufferSize) {
        for (Integer pID : parentSubSectionMap.keySet()) {
            ...
            if (!pArea.isSingular()) {
                System.out.println("    non-singular " + pID + " " + parentNameMap.get(pID));
            }
        }
    }
    ...
    private void build() {
        ...
        for (Integer id : polyMap.keySet()) {
            ...
            String mssg = (poly == null) ? "null" : (!poly.isSingular()) ? "non-singular" : "ok";
                ...
                if (fPoly.isSingular()) {
                    ...
                    for (LocationList locs : locLists) {
                        ...
                        if (intersects(trace, polyPart)) {
                            ...
                            if (prev == null) System.out.println("merge problem last segment");
    ...
    /**
     * Returns a flat-earth estimate of the area of this region in km<sup>2</sup>. Method uses the
     * center of this {@code Region}'s bounding polygon as the origin of an orthogonal coordinate
     * system. This method is not appropriate for use with very large {@code Region}s where the
     * curvature of the earth is more significant.
     *
     * <p>TODO should probably use centroid of polygon
     *
     * <p>Assumes aupplied area has already been cleaned of strays etc...
     *
