...
public class UCRuptureTester {
    ...
    public UCRuptureTester(String rupFileName) throws IOException {
        ...
        for (int i = 0; i < rupSet.getNumSections(); i++) {
            ...
            if (section.getSectionName().startsWith("Puysegur")
                    && section.getSectionName().contains("row:")
                    && puysegurStart == -1) {
                puysegurStart = i;
            }

            if (section.getSectionName().startsWith("Hikurangi")
                    && section.getSectionName().contains("row:")
                    && hikurangiStart == -1) {
                hikurangiStart = i;
            }
        ...
        for (FaultSection targetSection : target.buildOrderedSectionList()) {
            for (FaultSection nucleationSection : nucleation.clusters[0].subSects) {
                ...
                return new MultiRuptureJump(
                        nucleation.clusters[0].startSect,
                        nucleation,
                        target.clusters[0].startSect,
                        target,
                        distance);
            }
        }
        ...
        while (true) {
            ...
            if (!Objects.equals(ruptureId, currentRuptureId)) {
                ...
                if (currentRuptureId != null
                    ...
                    ruptures.put(
                            currentRuptureId,
                            makeJump(currentRuptures.get(0), currentRuptures.get(1)));
                }
            ...
            for (int i = 2; i < components.length; i++) {
                clusterSections.add(
                        rupSet.getFaultSectionData(
                                getSectionId(currentRuptureId, source, components[i])));
            }
            ClusterRupture rupture =
                    ClusterRupture.forOrderedSingleStrandRupture(clusterSections, disAzCalc);
    ...
    void setupStiffness() {
        ...
        stiffness =
                new StiffnessCalcModule(
                        rupSet,
                        2,
                        new File(
                                "C:\\Users\\user\\GNS\\rupture sets\\stiffnessCache-nzshm22_complete_merged\\"));
    ...
    public static void main(String[] args) throws IOException {
        UCRuptureTester ruptureTester =
                new UCRuptureTester(
                        "C:\\Users\\user\\GNS\\rupture sets\\nzshm22_complete_merged.zip");
        ruptureTester.loadCSV(
                "C:\\Users\\user\\GNS\\RSQSim\\AndyHowellAugust24\\biggest_events.txt");
