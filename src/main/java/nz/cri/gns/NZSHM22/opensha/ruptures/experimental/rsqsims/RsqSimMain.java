...
public class RsqSimMain implements Closeable {
    ...
    public RsqSimMain(
            String baseInputDir, String rundirVersion, SourceType sourceType, String baseOutputDir)
            throws IOException {
    ...
    public static void writeDebugGeoJSON(
            List<RsqSimEventLoader.Event> singleCrustalJointRuptures,
            List<RsqSimEventLoader.Event> passes,
            String baseOutput)
            throws IOException {
        ...
        int ruptureId = 0;
        for (RsqSimEventLoader.Event event :
                singleCrustalJointRuptures) { // List.of(ruptures.get(0), ruptures.get(1))) {
    ...
    public void process() throws IOException, FactoryException {
        ...
        List<RsqSimEventLoader.Event> singleCrustalJointRuptures =
                allSingleCrustalJointRuptures.stream()
                        .filter(
                                event ->
                                        event.sections.stream()
                                                        .filter(
        ...
        log(
                "- single crustal joint ruptures with crustal component >= 100km^2 "
                        + singleCrustalJointRuptures.size());

        eventLoader.writeParticipationRates(
                singleCrustalJointRuptures, patchLoader.loadedRupSet, baseOutputPath);

        CoulombTester tester =
                new CoulombTester(
                        patchLoader.loadedRupSet,
                        "C:\\tmp\\stiffnessCaches"); // "C:\\Users\\user\\GNS\\rupture
        // sets\\stiffnessCache-nzshm22_complete_merged\\");
        ...
        log(
                "- self stiffness > 0 single crustal joint ruptures: "
                        + tester.testSelfStiffnessFilter(singleCrustalJointRuptures));

        //  List<List<PlausibilityResult>> stiffness = ruptures.parallelStream().map(r ->
        // r.jump).map(tester::applyCoulomb).collect(Collectors.toList());
        // System.out.println("passes: " +stiffness.stream().map(s -> s.get(2).isPass()).filter(p ->
        // p).count());
        List<RsqSimEventLoader.Event> passes =
                allSingleCrustalJointRuptures.parallelStream()
                        .filter(event -> tester.applyCoulomb(event.jump).get(2).isPass())
                        .collect(Collectors.toList());
        log("- original filter passes: " + passes.size());

        List<ClusterRupture> clusterRuptures =
                singleCrustalJointRuptures.stream()
                        .map(event -> ManipulatedClusterRupture.makeRupture(event.sections))
                        .collect(Collectors.toList());

        FaultSystemRupSet resultRupSet =
                FaultSystemRupSet.builderForClusterRups(
                                patchLoader.loadedRupSet.getFaultSectionDataList(), clusterRuptures)
