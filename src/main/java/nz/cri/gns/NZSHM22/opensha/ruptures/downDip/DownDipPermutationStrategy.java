...
public class DownDipPermutationStrategy implements RuptureGrowingStrategy {
    ...
    private boolean applyConstraint(
            DownDipSubSectBuilder builder, int startRow, int startCol, int endRow, int endCol) {
        if (constraint == null) {
            return true;
        } else {
    ...
    private static FaultSubsectionCluster buildCopyJumps(
            FaultSubsectionCluster fullCluster, List<FaultSection> subsetSects) {
        FaultSubsectionCluster permutation =
                new FaultSubsectionCluster(new ArrayList<>(subsetSects));
        for (FaultSection sect : subsetSects)
            for (Jump jump : fullCluster.getConnections(sect))
                permutation.addConnection(
                        new Jump(sect, permutation, jump.toSection, jump.toCluster, jump.distance));
        return permutation;
    }

    private FaultSubsectionCluster buildRectangularPermutation(
        ...
        // this is a list of all exit points from this rupture (sections from which we can jump to
        // another
        // fault without it being considered a splay jump). we'll define that now as the far edges
        // from the
        // starting point, though other strategies likely exist
        List<FaultSection> exitPoints = new ArrayList<>();

        //		if (D) System.out.println("\t\trow span: "+startRow+" => "+endRow+":
        // "+printIndexes(startRow, endRow));
        //		if (D) System.out.println("\t\tcol span: "+startCol+" => "+endCol+":
        // "+printIndexes(startCol, endCol));
        //
        ...
        Preconditions.checkState(
                subsetSects.get(0).equals(downDipBuilder.getSubSect(startRow, startCol)));
        FaultSubsectionCluster permutation =
                new DownDipFaultSubSectionCluster(subsetSects, exitPoints);
    ...
    private static int[] indexes(int start, int end) {
        if (start <= end) {
            ...
            for (int i = 0; i < ret.length; i++) ret[i] = start + i;
            return ret;
        }
