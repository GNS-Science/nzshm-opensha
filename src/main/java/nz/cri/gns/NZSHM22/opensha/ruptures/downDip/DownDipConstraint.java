...
public interface DownDipConstraint {
    ...
    boolean apply(
            DownDipSubSectBuilder builder, int startRow, int startCol, int rowCount, int colCount);

    DownDipConstraint ALWAYS =
    ...
    /**
     * Verifies that there are no gaps in a rupture. The algorithm recursively visits a section and
     * all of its neighbours. Once it has visited all sections it can reach from the first
     * subsection, we assert that all sections of the rupture have been visited. Neighbours are only
     * visited orthogonally, not diagonally.
     *
    ...
    static DownDipConstraint connectednessConstraint() {
        return (DownDipSubSectBuilder builder,
                int startRow,
                int startCol,
                int rowCount,
                int colCount) -> {
            ...
            FaultSection initialSection =
                    findFirstSection(builder, startRow, startCol, rowCount, colCount);
            ...
            while (!toVisit.empty()) {
                ...
                for (FaultSection section : neighbours) {
                    ...
                    if (!seen.contains(section)) {
                        ...
                        // only put it on the stack if the section is actually inside the area that
                        // we are considering for this rupture.
                        if (col >= startCol
    ...
    /**
     * Returns an aspect ratio constraint that ensures that ruptures are within the specified
     * minRatio and maxRatio (incl). If a rupture starts at row 0 and has at least depthThreshold
     * rows, then maxRatio can be exceeded.
     *
     ...
     */
    static DownDipConstraint aspectRatioConstraint(
            double minRatio, double maxRatio, int depthThreshold) {
        return (DownDipSubSectBuilder builder,
                int startRow,
                int startCol,
                int rowCount,
    ...
    /**
     * Returns a constraint that ensures that the rectangular area that contains the rupture has
     * enough non-null subSections.
     *
     * @param minFill the ratio of subsections to the rectangle size (measured in the number of
     *     subSections)
     * @return the constraint
     */
    ...
    private static FaultSection findFirstSection(
            DownDipSubSectBuilder builder, int startRow, int startCol, int rowCount, int colCount) {
    ...
    // I believe sizeCoarsenessConstraint is meant to reduce the number of allowed sizes up until a
    // certain size,
    // and positionCoarsenessConstraint is meant to reduce the number of starting locations up until
    // a certain size.
    ...
    //            return (builder, startRow, startCol, rowCount, colCount) -> {
    //                int coarseness = Math.max(1, (int) Math.round(epsilon * rowCount * colCount));
    //                return isDivisibleBy(startCol, coarseness) && isDivisibleBy(startRow,
    // coarseness);
    //            };
    //        }
    //        return ALWAYS;
