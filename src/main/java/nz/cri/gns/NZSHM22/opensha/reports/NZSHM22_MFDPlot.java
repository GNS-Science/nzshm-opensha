...
public class NZSHM22_MFDPlot extends AbstractRupSetPlot {
    ...
    protected void plotNZSHM22_MFDs(
            List<MFD_Plot> plots, NZSHM22_CrustalInversionTargetMFDs targetMFDs) {

        SummedMagFreqDist targetOnFaultSupraSeisMFDs =
        ...
        totalPlot.addComp(
                targetMFDs.getTrulyOffFaultMFD(), Color.GRAY, "Target Gridded (TrulyOffFaultMFD)");
        totalPlot.addComp(
                targetMFDs.getTotalOnFaultSubSeisMFD(), Color.MAGENTA.darker(), "Target Sub-Seis");
        ...
        sansPlot.addComp(
                targetMFDs.getSansTvz().totalTargetGR, Color.GREEN.darker(), "Total Target GR");
        sansPlot.addComp(
                targetMFDs.getSansTvz().trulyOffFaultMFD,
                Color.GRAY,
                "Target Gridded (TrulyOffFaultMFD)");
        sansPlot.addComp(
                targetMFDs.getSansTvz().totalSubSeismoOnFaultMFD,
                Color.MAGENTA.darker(),
                "Target Sub-Seis");
        sansPlot.addComp(
                targetMFDs.getSansTvz().targetOnFaultSupraSeisMFDs,
                SUPRA_SEIS_TARGET_COLOR,
                "Target Supra-Seis");
        plots.add(sansPlot);
        ...
        if (!targetMFDs.getTvz().isEmpty) {
            ...
            tvzPlot.addComp(
                    targetMFDs.getTvz().trulyOffFaultMFD,
                    Color.GRAY,
                    "Target Gridded (TrulyOffFaultMFD)");
            tvzPlot.addComp(
                    targetMFDs.getTvz().totalSubSeismoOnFaultMFD,
                    Color.MAGENTA.darker(),
                    "Target Sub-Seis");
            tvzPlot.addComp(
                    targetMFDs.getTvz().targetOnFaultSupraSeisMFDs,
    ...
    public List<String> plot(
        ...
        if (rupSet.hasModule(InversionTargetMFDs.class)) {
            ...
            totalPlot.addComp(
                    targetMFDs.getTotalRegionalMFD(), Color.GREEN.darker(), "Total Target");
            ...
            for (IncrementalMagFreqDist constraint : constraints) {
                ...
                if (region == null || region.getName() == null || region.getName().isBlank()) {
                    if (constraints.size() == 1) name = "MFD Constraint";
                    else name = "MFD Constraint " + plots.size();
                } else {
                    name = region.getName();
                }
                ...
                for (Point2D pt : constraint)
                    if (pt.getY() > 1e-10)
                        minY =
                                Math.min(
                                        minY,
                                        Math.pow(10, Math.floor(Math.log10(pt.getY()) + 0.1)));
        ...
        for (MFD_Plot plot : plots) {
            ...
            for (int c = 0; c < plot.comps.size(); c++) {
                ...
                chars.add(
                        new PlotCurveCharacterstics(
                                PlotLineType.SOLID, 3f, plot.compColors.get(c)));
            }
            ...
            PlotSpec incrSpec =
                    new PlotSpec(
                            incrFuncs, chars, plot.name, "Magnitude", "Incremental Rate (per yr)");
            PlotSpec cmlSpec =
                    new PlotSpec(
                            cmlFuncs, chars, plot.name, "Magnitude", "Cumulative Rate (per yr)");
        ...
        for (int i = 0; i < plots.size(); i++) {
            ...
            double tick;
            if (xRange.getLength() > 3.5) tick = 0.5;
            else if (xRange.getLength() > 1.5) tick = 0.25;
            else tick = 0.1d;
    ...
    static Range xRange(IncrementalMagFreqDist mfd) {
        return new Range(
                mfd.getMinX() - 0.5 * mfd.getDelta(), mfd.getMaxX() + 0.5 * mfd.getDelta());
    }
    ...
    private static Color avg(Color c1, Color c2) {
        ...
        return new Color(
                (int) Math.round(r * 0.5d), (int) Math.round(g * 0.5d), (int) Math.round(b * 0.5d));
    }

    private static double addSolMFDs(
            FaultSystemSolution sol,
            String name,
            Color color,
            Region region,
            List<IncrementalMagFreqDist> incrFuncs,
            List<EvenlyDiscretizedFunc> cmlFuncs,
            List<PlotCurveCharacterstics> chars,
            IncrementalMagFreqDist defaultMFD) {
        IncrementalMagFreqDist mfd =
                sol.calcNucleationMFD_forRegion(
                        region,
                        defaultMFD.getMinX(),
                        defaultMFD.getMaxX(),
                        defaultMFD.size(),
        ...
        if (sol.hasModule(GridSourceProvider.class)) {
            ...
            boolean regionTest =
                    region != null
                            && region != gridReg
                            && !region.getBorder().equals(gridReg.getBorder());
            for (int i = 0; i < gridReg.getNodeCount(); i++) {
                IncrementalMagFreqDist nodeMFD = prov.getMFD(i);
                if (nodeMFD == null) continue;
                if (regionTest && !region.contains(gridReg.getLocation(i))) continue;
                if (gridMFD == null)
                    gridMFD =
                            new SummedMagFreqDist(
                                    nodeMFD.getMinX(), nodeMFD.getMaxX(), nodeMFD.size());
            ...
            if (gridMFD != null) {
                if (!name.toLowerCase().contains("comparison")) {
                    ...
                    chars.add(
                            new PlotCurveCharacterstics(
                                    PlotLineType.DASHED, 3f, avg(color, Color.WHITE)));
                    //					chars.add(new PlotCurveCharacterstics(PlotLineType.DASHED, 3f,
                    // color.brighter()));
                }
                SummedMagFreqDist totalMFD =
                        new SummedMagFreqDist(mfd.getMinX(), mfd.getMaxX(), mfd.size());
