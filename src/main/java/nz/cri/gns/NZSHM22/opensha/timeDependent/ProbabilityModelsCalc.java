...
/**
 * Copied from scratch.UCERF3.erf.utils.ProbabilityModelsCalc
 *
 * <p>This class does various time-dependent earthquake probability calculations for ruptures in a
 * fault system solution (FSS).
 *
 * <p>TODO:
 *
 * <p>0) implement the getU3_ProbGain2_ForRup method
 *
 * <p>1) junit tests
 *
 * <p>2) improve efficiency?
 *
 * <p>3) remove the "OLD*" methods here, as they were experimental and are no longer needed.
 *
 * <p>4) enable changing just the timeSpan and/or aperiodicity (would require setting dependent
 * arrays to null)
 *
 * @author field
 */
public class ProbabilityModelsCalc {
    ...
    public static final double MILLISEC_PER_YEAR = 1000 * 60 * 60 * 24 * 365.25;
    public static final long MILLISEC_PER_DAY = 1000 * 60 * 60 * 24;

    // passed in values:
    FaultSystemSolutionERF erf;
    FaultSystemSolution fltSysSolution;
    double[] longTermRateOfFltSysRup; // this has zeros where events were filtered our by the ERF
    // (mags too low); this includes aftershocks
    //	double aperiodicity;
    ...
    // The following ave recurrence interval of each rupture conditioned on the fact that it is the
    // next event to occur
    double[] aveCondRecurIntervalForFltSysRups_type1; //  for averaging section recurrence intervals
    // and time since last;
    double[] aveCondRecurIntervalForFltSysRups_type2; //  for averaging section rates and normalized
    // time since last
    ...
    // this is for getting the BPT time since last that is equivalent to the poisson probability
    ...
    static final File dataDir =
            new File(UCERF3_DataUtils.DEFAULT_SCRATCH_DATA_DIR, File.separator + "erSimulations");
    ...
    public ProbabilityModelsCalc(
            FaultSystemSolution fltSysSolution,
            double[] longTermRateOfFltSysRupInERF,
            MagDependentAperiodicityOptions magDepAperiodicity) {
        this(
                fltSysSolution,
                longTermRateOfFltSysRupInERF,
                magDepAperiodicity.getAperValuesArray(),
                magDepAperiodicity.getAperMagBoundariesArray());
    }
    ...
    public ProbabilityModelsCalc(
        ...
        this(
                fltSysSolution,
                longTermRateOfFltSysRupInERF,
                aperiodicity.getAperValuesArray(),
                aperiodicity.getAperMagBoundariesArray());
    }
     ...
     * small events (TODO not entirely sure this is necessary; depends on whether small sections
     * should be included in total section rates; the same filtering could also be done here).
     *
     ...
     *
     * <p>However, it can't be used if ProbabilityModelParam.NAME).getValue() ==
    ...
    public ProbabilityModelsCalc(FaultSystemSolutionERF erf) {
        ...
        if (erf.getParameter(ProbabilityModelParam.NAME).getValue()
                == ProbabilityModelOptions.U3_PREF_BLEND)
            throw new RuntimeException(
                    "This constructor can't be used with " + ProbabilityModelOptions.U3_PREF_BLEND);

        MagDependentAperiodicityOptions magDepAperiodicity = null;
        ;
        if (erf.getAdjustableParameterList()
                .containsParameter(MagDependentAperiodicityParam.NAME)) {
            magDepAperiodicity =
                    ((MagDependentAperiodicityParam)
                                    erf.getParameter(MagDependentAperiodicityParam.NAME))
                            .getValue();
    ...
    public ProbabilityModelsCalc(double aperiodicity) {
        ...
        //		BPT_DistCalc tempCalc = new BPT_DistCalc();
        //		double delta = max_time_for_normBPT_CDF/(num_for_normBPT_CDF-1);
        //		tempCalc.setAll(1.0, aperiodicity, delta, num_for_normBPT_CDF);	// TODO check this
        // discretization and overall look
        //		normBPT_CDF=tempCalc.getCDF();
    ...
    /**
     * This computes average conditional recurrent intervals for each fault system rup (the
     * recurrence interval assuming the rup is the next to occur), either by averaging section
     * recurrence intervals (typeCalc=1) or by computing one over the average section rate
     * (typeCalc=2), both are weighted by section area.
     *
        ...
        for (int r = 0; r < numRupsInFaultSystem; r++) {
            ...
            for (FaultSection data : fltData) {
                ...
                if (typeCalc == 1)
                    ave +=
                            area
                                    / longTermPartRateForSectArray[
                                            sectID]; // this one averages RIs; wt averaged by area
                else if (typeCalc == 2)
                    ave +=
                            longTermPartRateForSectArray[sectID]
                                    * area; // this one averages rates; wt averaged by area
            ...
            if (typeCalc == 1)
                ...
                aveCondRecurIntervalForFltSysRups[r] =
                        1 / (ave / totArea); // this one averages rates
            // temp overide:
    ...
    /**
     * This computes average conditional recurrent interval using only sections that lack a date of
     * last event, either by averaging section recurrence intervals (aveRI_CalcType=true) or by
     * computing one over the average section rate (aveRI_CalcType=false), both are weighted by
     * section area. Double.NaN is returned if all sections have a date of last event.
     *
     * @return
     */
    private double computeAveCondRecurIntForFltSysRupsWhereDateLastUnknown(
            int fltSysRupIndex, boolean aveRI_CalcType, long presentTimeMillis) {
        ...
        if (totArea == 0.0) return Double.NaN;
        else if (aveRI_CalcType) return ave / totArea; // this one averages RIs
        else return 1.0 / (ave / totArea); // this one averages rates
    }

    /**
     * This method returns last-event date (epoch milliseconds) averaged over fault sections that
     * have such data (and weighted by section area).
     *
     * <p>Long.MIN_VALUE is returned if none of the fault sections had a date of last event
     *
     * <p>The following global variables are also set for further diagnostics:
     *
     * <p>double totRupArea double totRupAreaWithDateOfLast boolean allSectionsHadDateOfLast boolean
     * noSectionsHadDateOfLast
     *
     * @param fltSystRupIndex
     * @param presentTimeMillis if non null, only fault sections with date of last events at or
     *     before the present time in milliseconds will be considered.
     */
    public long getAveDateOfLastEventWhereKnown(int fltSystRupIndex, Long presentTimeMillis) {
        ...
        for (int s : fltSysRupSet.getSectionsIndicesForRup(fltSystRupIndex)) {
            ...
            if (dateOfLast != Long.MIN_VALUE
                    && (presentTimeMillis == null || dateOfLast <= presentTimeMillis)) {
                // System.out.println("dateOfLast="+dateOfLast+";
                // presentTimeMillis="+presentTimeMillis);
    ...
    /**
     * This method returns normalized time since last event (timeSince/meanRecurInt) averaged over
     * fault sections that have such data (and weighted by section area).
     *
     * <p>Double.NaN is returned in none of the fault sections have a date of last event
     *
     * <p>The following global variables are also set for further diagnostics:
     *
     * <p>double totRupArea double totRupAreaWithDateOfLast boolean allSectionsHadDateOfLast boolean
     ...
     */
    public double getAveNormTimeSinceLastEventWhereKnown(
            int fltSystRupIndex, long presentTimeMillis) {

        //		System.out.println("getAveNormTimeSinceLastEventWhereKnown");

        //		List<FaultSectionPrefData> fltData =
        // fltSysRupSet.getFaultSectionDataForRupture(fltSystRupIndex);
        totRupArea = 0;
        totRupAreaWithDateOfLast = 0;
        allSectionsHadDateOfLast = true;
        int numWithDateOfLast = 0;
        ...
        for (int s : fltSysRupSet.getSectionsIndicesForRup(fltSystRupIndex)) {
            ...
            if (dateOfLast != Long.MIN_VALUE && dateOfLast <= presentTimeMillis) {
                sumNormTimeSinceLast +=
                        area
                                * ((double) (presentTimeMillis - dateOfLast) / MILLISEC_PER_YEAR)
                                * longTermPartRateForSectArray[s];
                totRupAreaWithDateOfLast += area;
                numWithDateOfLast += 1;
            } else {
                allSectionsHadDateOfLast = false;
        ...
        if (numWithDateOfLast > 0) return sumNormTimeSinceLast / totRupAreaWithDateOfLast;
        else {
            noSectionsHadDateOfLast = true;
            return Double.NaN;
        }
    ...
    public int writeSectionsWithDateOfLastEvent() {
        ...
        int numWith = 0;
        System.out.println(
                "Sections With Date of Last Event Data (timeSinceLastYears, dateOfLastMillis, sectName):");
        for (FaultSection data : fltData) {
        ...
        try {
            ...
            for (int r = 0; r < fltSysRupSet.getNumRuptures(); r++) {
                if (fltSysRupSet.getSectionsIndicesForRup(r).contains(sectID)) {
                    ...
                    double yrsSinceLast =
                            ((2014 - 1970) * MILLISEC_PER_YEAR - (double) aveDateOfLastMillis)
                                    / MILLISEC_PER_YEAR;
                    ...
                    String name2 =
                            fltSysRupSet
                                    .getFaultSectionData(indices.get(indices.size() - 1))
                                    .getName();
                    String line =
                            r
                                    + "\t"
                                    ...
                                    + yrsSinceLast
                                    + "\t"
                                    + (totRupAreaWithDateOfLast / totRupArea)
                                    + "\t"
                                    + name1
                                    + "\t"
                                    + name2
     ...
     *
     * <p>Gain is defined as the BPT probability divided by the expected number
     * (durationYears/aveCondRecurInterval); (dividing by Poisson probability gives biased results
     * for long durations). Note that if the expected number exceeds 1.0, the gain becomes less than
     * 1.0 (since probability can't exceed 1.0), so maybe this shouldn't be called "gain".
     *
     * <p>This returns Double.NaN if onlyIfAllSectionsHaveDateOfLast=true and one or mare sections
     * lack date of last event.
     *
     * @param onlyIfAllSectionsHaveDateOfLast
     * @param fltSystRupIndex
     * @param histOpenInterval
     * @param aveRecurIntervals - if false, rates will be averaged in get the conditional recurrence
     *     interval
     * @param aveNormTimeSinceLast - if true, normalized time since last is averaged (divided by
     *     section intervals); otherwise time since last is averaged
     */
    public double getU3_ProbGainForRup(
            int fltSysRupIndex,
            double histOpenInterval,
            boolean onlyIfAllSectionsHaveDateOfLast,
            boolean aveRecurIntervals,
            boolean aveNormTimeSinceLast,
            long presentTimeMillis,
        ...
        if (aveRecurIntervals) {
            if (aveCondRecurIntervalForFltSysRups_type1 == null)
                aveCondRecurIntervalForFltSysRups_type1 =
                        computeAveCondRecurIntervalForFltSysRups(1);
            ...
            if (aveCondRecurIntervalForFltSysRups_type2 == null)
                aveCondRecurIntervalForFltSysRups_type2 =
                        computeAveCondRecurIntervalForFltSysRups(2);
        ...
        if (aveNormTimeSinceLast) {
            aveNormTimeSinceLastEventWhereKnown =
                    getAveNormTimeSinceLastEventWhereKnown(fltSysRupIndex, presentTimeMillis);
            aveTimeSinceLastWhereKnownYears =
                    aveNormTimeSinceLastEventWhereKnown * aveCondRecurInterval;
            //			if(aveTimeSinceLastWhereKnownYears<0)
            //				throw new RuntimeException("1st "+aveTimeSinceLastWhereKnownYears);
        } else {
            long aveTimeOfLastMillisWhereKnown =
                    getAveDateOfLastEventWhereKnown(fltSysRupIndex, presentTimeMillis);
            if (aveTimeOfLastMillisWhereKnown != Long.MIN_VALUE)
                aveTimeSinceLastWhereKnownYears =
                        (double) (presentTimeMillis - aveTimeOfLastMillisWhereKnown)
                                / MILLISEC_PER_YEAR;
            else aveTimeSinceLastWhereKnownYears = Double.NaN;
            //			if(aveTimeSinceLastWhereKnownYears<0)
            //				throw new RuntimeException("2nd
            // "+aveTimeSinceLastWhereKnownYears+"\t"+presentTimeMillis+"\t"+aveTimeOfLastMillisWhereKnown+"\t"+Long.MIN_VALUE);

        }

        Preconditions.checkState(
                Double.isNaN(aveTimeSinceLastWhereKnownYears)
                        || aveTimeSinceLastWhereKnownYears >= 0,
                "aveTimeSinceLastWhereKnownYears=%s",
                aveTimeSinceLastWhereKnownYears);
        ...
        if (onlyIfAllSectionsHaveDateOfLast && !allSectionsHadDateOfLast) {
            ...
            probGain =
                    computeBPT_ProbForUnknownDateOfLastFast(
                                    aveCondRecurInterval, histOpenInterval, durationYears, rupMag)
                            / expNum;
            // if(fltSysRupIndex==testRupID) System.out.println("Here3");
        } else { // case where some have date of last; loop over all possibilities for those that
            // don't.
            ...
            double condRecurIntWhereUnknown =
                    computeAveCondRecurIntForFltSysRupsWhereDateLastUnknown(
                            fltSysRupIndex, aveRecurIntervals, presentTimeMillis);
            ...
            if (aveNormTimeSinceLast) {
                for (int i = 0; i < normBPT_CDF.size(); i++) {
                    ...
                    double relProbForTimeSinceLast =
                            1.0
                                    - normBPT_CDF.getY(
                                            i); // this is the probability of the date of last event
                    // (not considering hist open interval)
                    if (normTimeSinceYears * condRecurIntWhereUnknown >= histOpenInterval
                            && relProbForTimeSinceLast > 1e-15) {
                        double aveNormTS =
                                (normTimeSinceYears * areaWithOutDateOfLast
                                                + aveNormTimeSinceLastEventWhereKnown
                                                        * totRupAreaWithDateOfLast)
                                        / totRupArea;
                        double condProb =
                                computeBPT_ProbFast(
                                        1.0,
                                        aveNormTS,
                                        durationYears / aveCondRecurInterval,
                                        rupMag);
                ...
                for (int i = 0; i < normBPT_CDF.size(); i++) {
                    ...
                    if (timeSinceYears >= histOpenInterval && relProbForTimeSinceLast > 1e-15) {
                        ...
                        double aveTimeSinceLast =
                                (timeSinceYears * areaWithOutDateOfLast
                                                + aveTimeSinceLastWhereKnownYears
                                                        * totRupAreaWithDateOfLast)
                                        / totRupArea;
                        double condProb =
            ...
            if (totWeight > 0) {
            ...
            } else { // deal with case where there was no viable time since last; use exactly
                // historic open interval
                // List<FaultSectionPrefData> fltDataList =
                // fltSysRupSet.getFaultSectionDataForRupture(fltSysRupIndex);
                // System.out.println("FIXING: "+fltDataList.get(0).getName()+" to
                // "+fltDataList.get(fltDataList.size()-1).getName()+
                ...
                if (aveNormTimeSinceLast) {
                    ...
                    double aveNormTS =
                            (normTimeSinceYearsUnknown * areaWithOutDateOfLast
                                            + aveNormTimeSinceLastEventWhereKnown
                                                    * totRupAreaWithDateOfLast)
                                    / totRupArea;
                    double condProb =
                            computeBPT_ProbFast(
                                    1.0, aveNormTS, durationYears / aveCondRecurInterval, rupMag);
                    probGain = condProb / expNum;
                } else {
                    double aveTimeSinceLast =
                            (histOpenInterval * areaWithOutDateOfLast
                                            + aveTimeSinceLastWhereKnownYears
                                                    * totRupAreaWithDateOfLast)
                                    / totRupArea;
                    double condProb =
                            computeBPT_ProbFast(
                                    aveCondRecurInterval, aveTimeSinceLast, durationYears, rupMag);
                    probGain = condProb / expNum;
                }
            }
        }
        ...
        if (simulationMode) {
            if (aveTimeSinceLastWhereKnownYears / aveCondRecurInterval
                    > simNormTimeSinceLastHist.getMaxX()) {
                simNormTimeSinceLastHist.add(
                        simNormTimeSinceLastHist.getMaxX(),
                        longTermRateOfFltSysRup[fltSysRupIndex]);
                if (rupMag <= 7)
                    simNormTimeSinceLastForMagBelow7_Hist.add(
                            simNormTimeSinceLastHist.getMaxX(),
                            longTermRateOfFltSysRup[fltSysRupIndex]);
                else
                    simNormTimeSinceLastForMagAbove7_Hist.add(
                            simNormTimeSinceLastHist.getMaxX(),
                            longTermRateOfFltSysRup[fltSysRupIndex]);
            } else {
                simNormTimeSinceLastHist.add(
                        aveTimeSinceLastWhereKnownYears / aveCondRecurInterval,
                        longTermRateOfFltSysRup[fltSysRupIndex]);
                if (rupMag <= 7)
                    simNormTimeSinceLastForMagBelow7_Hist.add(
                            aveTimeSinceLastWhereKnownYears / aveCondRecurInterval,
            ...
            if (probGain > simProbGainHist.getMaxX()) {
                simProbGainHist.add(
                        simProbGainHist.getMaxX(), longTermRateOfFltSysRup[fltSysRupIndex]);
                if (rupMag <= 7)
                    simProbGainForMagBelow7_Hist.add(
                            simProbGainHist.getMaxX(), longTermRateOfFltSysRup[fltSysRupIndex]);
                else
                    simProbGainForMagAbove7_Hist.add(
                            simProbGainHist.getMaxX(), longTermRateOfFltSysRup[fltSysRupIndex]);
            } else {
                simProbGainHist.add(probGain, longTermRateOfFltSysRup[fltSysRupIndex]);
                if (rupMag <= 7)
                    simProbGainForMagBelow7_Hist.add(
                            probGain, longTermRateOfFltSysRup[fltSysRupIndex]);
                else
                    simProbGainForMagAbove7_Hist.add(
                            probGain, longTermRateOfFltSysRup[fltSysRupIndex]);
            }
     ...
     * gain of each sections is averaged, weighted by section area (actually, this should be
     * weighted by moment rate, but tests show no difference).
     *
     * <p>This returns Double.NaN if onlyIfAllSectionsHaveDateOfLast=true and one or mare sections
     * lack date of last event.
     *
     ...
     * @param onlyIfAllSectionsHaveDateOfLast
     */
    public double getWG02_ProbGainForRup(
            int fltSysRupIndex,
            boolean onlyIfAllSectionsHaveDateOfLast,
            long presentTimeMillis,
            double durationYears) {
        ...
        if (sectionGainArray == null) {
            ...
            for (int s = 0; s < numSections; s++) {
                ...
                if (timeOfLastMillis != Long.MIN_VALUE && timeOfLastMillis <= presentTimeMillis) {
                    double timeSinceLastYears =
                            ((double) (presentTimeMillis - timeOfLastMillis)) / MILLISEC_PER_YEAR;
                    double refTimeSinceLast =
                            timeSinceLastYears * refRI * longTermPartRateForSectArray[s];
                    double refDuration = durationYears * refRI * longTermPartRateForSectArray[s];
                    double prob_bpt =
                            refBPT_DistributionCalc.getCondProb(refTimeSinceLast, refDuration);
                    //					double prob_pois =
                    // 1-Math.exp(-durationYears*longTermPartRateForSectArray[s]);
                    double prob_pois =
                            durationYears
                                    * longTermPartRateForSectArray[
                                            s]; // this is there exact calculation, which is a bit
                    // different for long durations
        ...
        if (noneAreReal) return 1d;
        else return sumGains / totalWt;
    }
    ...
    public void plotXYZ_FuncOfCondProbForUnknownDateOfLastEvent() {
        ...
        int numLogDurOverMean =
                1 + (int) Math.ceil((maxLogDurOverMean - minLogDurOverMean) / deltaLogDurOverMean);
        ...
        int numLogHistOpenIntOverMean =
                1
                        + (int)
                                Math.ceil(
                                        (maxLogHistOpenIntOverMean - minLogHistOpenIntOverMean)
                                                / deltaLogHistOpenIntOverMean);

        // this is what we will return
        EvenlyDiscrXYZ_DataSet xyzDataCondProbForUnknown =
                new EvenlyDiscrXYZ_DataSet(
                        numLogDurOverMean,
                        numLogHistOpenIntOverMean,
                        minLogDurOverMean,
                        minLogHistOpenIntOverMean,
                        deltaLogDurOverMean,
                        deltaLogHistOpenIntOverMean);

        EvenlyDiscrXYZ_DataSet xyzDataProbGain =
                xyzDataProbGain =
                        new EvenlyDiscrXYZ_DataSet(
                                numLogDurOverMean,
                                numLogHistOpenIntOverMean,
        ...
        for (int y = 0; y < xyzDataCondProbForUnknown.getNumY(); y++) {
            ...
            for (int x = 0; x < xyzDataCondProbForUnknown.getNumX(); x++) {
                ...
                double condProbForUnknownTimeSinceLast =
                        bptCalc2.getCondProbForUnknownTimeSinceLastEvent();
                xyzDataCondProbForUnknown.set(x, y, Math.log10(condProbForUnknownTimeSinceLast));
                double probGain =
                        condProbForUnknownTimeSinceLast / computePoissonProb(refRI, duration);
        ...
        try {
            ...
            //			cpt_prob =
            // GMT_CPT_Files.MAX_SPECTRUM.instance().rescale(xyzDataCondProbForUnknown.getMinZ(),
            // 0);
            //			cpt_probGain =
            // GMT_CPT_Files.MAX_SPECTRUM.instance().rescale(xyzDataProbGain.getMinZ(),
            // xyzDataProbGain.getMaxZ());
            //			System.out.println("\t condProb min &
            // max:\t"+(float)xyzDataCondProbForUnknown.getMinZ()+"\t"+(float)xyzDataCondProbForUnknown.getMaxZ());
            //			System.out.println("\t probGain min &
            // max:\t"+(float)xyzDataProbGain.getMinZ()+"\t"+(float)xyzDataProbGain.getMaxZ());
        } catch (IOException e) {
            e.printStackTrace();
        }
        XYZPlotSpec spec_prob =
                new XYZPlotSpec(
                        xyzDataCondProbForUnknown,
                        cpt_prob,
                        "CondProbForUnknownLast; aper=" + (float) aperiodicity,
                        "LogNormDuration",
                        "LogNormHistOpenInt",
                        "Probability");
        GraphWindow window_prob = new GraphWindow(spec_prob);
        XYZPlotSpec spec_probGain =
                new XYZPlotSpec(
                        xyzDataProbGain,
                        cpt_probGain,
                        "Log10 Prob Gain (vs Poisson); aper=" + (float) aperiodicity,
                        "LogNormDuration",
                        "LogNormHistOpenInt",
                        "Log10 Prob Gain");
    ...
    public void plotXYZ_FuncOfCondProb() {
        ...
        int numLogNormTimeSinceLast =
                1
                        + (int)
                                Math.ceil(
                                        (maxLogNormTimeSinceLast - minLogNormTimeSinceLast)
                                                / deltaLogNormTimeSinceLast);

        EvenlyDiscrXYZ_DataSet xyzDataCondProb =
                new EvenlyDiscrXYZ_DataSet(
                        numLogNormTimeSinceLast,
                        numLogDurOverMean,
                        minLogNormTimeSinceLast,
                        minLogDurOverMean,
                        deltaLogNormTimeSinceLast,
                        deltaLogDurOverMean);
        ...
        for (int x = 0; x < xyzDataCondProb.getNumX(); x++) {
            ...
            for (int y = 0; y < xyzDataCondProb.getNumY(); y++) {
                ...
                if (condProb == 0) condProb = Double.NaN;
        ...
        try {
            ...
            cpt_prob =
                    GMT_CPT_Files.MAX_SPECTRUM
                            .instance()
                            .rescale(xyzDataCondProb.getMinZ(), xyzDataCondProb.getMaxZ());
            cpt_probGain =
                    GMT_CPT_Files.MAX_SPECTRUM
                            .instance()
                            .rescale(xyzDataProbGain.getMinZ(), xyzDataProbGain.getMaxZ());
        } catch (IOException e) {
            e.printStackTrace();
        }
        XYZPlotSpec spec_prob =
                new XYZPlotSpec(
                        xyzDataCondProb,
                        cpt_prob,
                        "Log10 BPT Cond Prob; aper=" + (float) aperiodicity,
                        "LogNormTimeSinceLast",
                        "LogNormDuration",
                        "Probability");
        GraphWindow window_prob = new GraphWindow(spec_prob);
        XYZPlotSpec spec_probGain =
    ...
    /**
     * This is made fast by using a reference calculator (with a reference RI), rather than redoing
     ...
     * @return
     */
    public double computeBPT_ProbFast(
            double aveRecurIntervalYears,
            double aveTimeSinceLastYears,
            double durationYears,
            double rupMag) {
        ...
        if (newTimeSinceLast < 0 && newTimeSinceLast > -1e-10) newTimeSinceLast = 0;
        double prob =
                refBPT_DistributionCalc.getCondProb(
                        newTimeSinceLast, durationYears * refRI / aveRecurIntervalYears);
        //		if(prob<0d)
        //			System.out.println("Negative Prob:
        // "+prob+"\t"+aveRecurIntervalYears+"\t"+aveTimeSinceLastYears+"\t"+durationYears);
        return prob;
    }
     ...
     * @return
     */
    public double computeBPT_ProbForUnknownDateOfLastFast(
            double aveRecurIntervalYears,
            double histOpenIntervalYears,
            double durationYears,
            double rupMag) {
        BPT_DistCalc refBPT_DistributionCalc = refBPT_CalcArray[getAperIndexForRupMag(rupMag)];
        refBPT_DistributionCalc.setDurationAndHistOpenInterval(
                durationYears * refRI / aveRecurIntervalYears,
                histOpenIntervalYears * refRI / aveRecurIntervalYears);
     ...
     * @return
     */
    public double computeBPT_ProbForUnknownDateOfLast(
            double aveRecurIntervalYears,
            double histOpenIntervalYears,
            double durationYears,
            double aperiodicity) {
        ...
        bptCalc.setInterpolate(interpolate);
        bptCalc.setAll(
                aveRecurIntervalYears,
                aperiodicity,
                delta,
                numPts,
                durationYears,
                histOpenIntervalYears);
    ...
    /**
     * This creates a reference BPT distribution calculators for the given aperiodicities and
     * duration (the calculator uses refRI and deltaT)
     *
     * @param bpt_Aperiodicity
    ...
    protected int getAperIndexForRupMag(double rupMag) {
        int index = -1;
        if (numAperValues == 1) index = 0; // only one
        else if (rupMag
                > aperMagBoundaries[
                        numAperValues
                                - 2]) // minus 2 to get last value since aperMagBoundaries has one
            // less element
    ...
    protected String getMagDepAperInfoString(int aperIndex) {
        ...
        if (aperIndex == 0) { // first one
            ...
            return "M>"
                    + aperMagBoundaries[numAperValues - 2]
                    + "; aper="
                    + aperValues[numAperValues - 1];
        } else { // intermediate one
            return aperMagBoundaries[aperIndex - 1]
                    + "<M<="
                    + aperMagBoundaries[aperIndex]
                    + "; aper="
                    + aperValues[aperIndex];
        }
    }
    ...
    /**
     * This test the slow versus fact computations here, monte carlo sampling over a range of aver
     * recur intervals, normalized durations, normalized time since last, and normalize historic
     * open intervals. Fractional discrepancies greater than 0.001 are listed. All are generally
     * small and for large normalized durations or historic open intervals, where differences in how
     * to avoid numerical problems arise. I don't think any of these are significant for UCERF3, but
     * the better test for the latter would be to do it both ways for UCERF3 (test the long way
     * against the fast caclulations).
     *
     * @param numTests
     */
    public static void testFastCalculations(int numTests) {
        ...
        System.out.println(
                "Prob\tdiff\tprob\tprob_fast\taperiodicity\taveRI\tdur\ttimeSince\thistOpenInt\tnormDur\tnormTimeSince\tnormHistOpenInt");
        ...
        for (double aper : apers) {
            ...
            for (int i = 0; i < numTests; i++) {
                ...
                double normDur =
                        0.001
                                + Math.random()
                                        * 5.0; // get normalized duration between 0.001 and 5.001
                double normTimeSince =
                        Math.random() * 5.0; // get normalized time since last between and 5
                ...
                double rupMag =
                        Double.NaN; // this will be ignored because there is only one aperiodicity
                // here

                double prob1 = calc.computeBPT_Prob(aveRI, timeSince, dur, aper);
                double prob2 =
                        calc.computeBPT_ProbForUnknownDateOfLast(aveRI, histOpenInt, dur, aper);
                double prob1_fast = calc.computeBPT_ProbFast(aveRI, timeSince, dur, rupMag);
                double prob2_fast =
                        calc.computeBPT_ProbForUnknownDateOfLastFast(
                                aveRI, histOpenInt, dur, rupMag);

                double diff1 = Math.abs((prob1 - prob1_fast) / prob1);
                if (prob1 < 1e-12 && prob1_fast < 1e-12) diff1 = 0;

                double diff2 = Math.abs((prob2 - prob2_fast) / prob2);
                if (prob2 < 1e-12 && prob2_fast < 1e-12) diff2 = 0;
     ...
     *
     * <p>This assumes the rate of each rupture is constant up until the next event is sampled.
     *
     * <p>TODO:
     *
     * <p>Shouldn't pass in erf (use the local one)
     *
     * <p>Have this ignore non-fault=system ruptures (others take way too long this way).
     *
     * <p>add progress bar
     */
    public void testER_Simulation(
            String inputDateOfLastFileName,
            String outputDateOfLastFileName,
            FaultSystemSolutionERF erf,
            double numYears,
            String dirNameSuffix) {
        ...
        if (aveRecurIntervals) typeCalcForU3_Probs = "aveRI";
        else typeCalcForU3_Probs = "aveRate";
        if (aveNormTimeSinceLast) typeCalcForU3_Probs += "_aveNormTimeSince";
        else typeCalcForU3_Probs += "_aveTimeSince";

        //		String aperiodicity="DefaultRange";
        //		String aperString = "aper"+aperiodicity;

        String probTypeString;
        ProbabilityModelOptions probTypeEnum =
                (ProbabilityModelOptions) erf.getParameter(ProbabilityModelParam.NAME).getValue();

        String aperString = "aper";
        if (probTypeEnum != ProbabilityModelOptions.POISSON) {
            boolean first = true;
            for (double aperVal : this.aperValues) {
                if (first) aperString += aperVal;
                else aperString += "," + aperVal;
                first = false;
            }
        ...
        } else throw new RuntimeException("Porbability type unrecognized");
        ...
        System.out.println(
                "orig start time: " + origStartTimeMillis + " millis (" + origStartYear + " yrs)");
        ...
        ArrayList<ArrayList<Double>> normalizedRupRecurIntervalsMagDepList =
                new ArrayList<ArrayList<Double>>();
        ArrayList<ArrayList<Double>> normalizedSectRecurIntervalsMagDepList =
                new ArrayList<ArrayList<Double>>();
        for (int i = 0; i < numAperValues; i++) {
        ...
        ArbDiscrEmpiricalDistFunc_3D normRI_AlongStrike =
                new ArbDiscrEmpiricalDistFunc_3D(0.05d, 0.95d, 10);
        ...
        double[] aveRupProbGainArray =
                new double[erf.getTotNumRups()]; // averages the prob gains at each event time
        double[] minRupProbGainArray =
                new double[erf.getTotNumRups()]; // averages the prob gains at each event time
        double[] maxRupProbGainArray =
                new double[erf.getTotNumRups()]; // averages the prob gains at each event time
        ...
        int numRups = 0;
        //		RandomDataImpl randomDataSampler = new RandomDataImpl();	// old apache tool for sampling
        // from exponential distribution here
        ...
        double totalRate = 0;
        IntegerPDF_FunctionSampler nthRupRandomSampler =
                new IntegerPDF_FunctionSampler(erf.getTotNumRups());
        double[] longTermRateOfNthRups =
                new double[erf.getTotNumRups()]; // this will include any aftershock reductions
                    ...
                    if (fltSysRupSet instanceof InversionFaultSystemRupSet) {
                        slips =
                                ((InversionFaultSystemRupSet) fltSysRupSet)
                                        .getSlipOnSectionsForRup(
                                                erf.getFltSysRupIndexForNthRup(nthRup));
                    } else { // apply ave to all sections
                        double mag =
                                fltSysRupSet.getMagForRup(erf.getFltSysRupIndexForNthRup(nthRup));
                        double area =
                                fltSysRupSet.getAreaForRup(erf.getFltSysRupIndexForNthRup(nthRup));
                        double aveSlip = FaultMomentCalc.getSlip(area, MagUtils.magToMoment(mag));
                        slips = new double[sectIndices.size()];
                        for (int i = 0; i < slips.length; i++) slips[i] = aveSlip;
                    }
                    for (int s = 0; s < sectIndices.size(); s++) {
                        int sectID = sectIndices.get(s);
        ...
        for (int s = 0; s < erf.getNumFaultSystemSources(); s++) {
            List<Integer> indexList =
                    fltSysRupSet.getSectionsIndicesForRup(erf.getFltSysRupIndexForSource(s));
        ...
        //			dateOfLastForSect[i] =
        // currentTimeMillis-Math.round(1.0*MILLISEC_PER_YEAR/longTermPartRateForSectArray[i]);
        // *****************************************************
        ...
        while (currentYear < numYears + origStartYear) {
            ...
            progressBar.updateProgress(
                    (int) Math.round(currentYear - origStartYear), (int) Math.round(numYears));

            // write progress
            int percDone = (int) Math.round(100 * (currentYear - origStartYear) / numYears);
            if (percDone >= percDoneThresh) {
                double timeInMin =
                        ((double) (System.currentTimeMillis() - startRunTime) / (1000.0 * 60.0));
                int numGoodDateOfLast = 0;
                for (long dateOfLast : dateOfLastForSect) {
                    if (dateOfLast != Long.MIN_VALUE) numGoodDateOfLast += 1;
                }
                int percentGood =
                        (int)
                                Math.round(
                                        (100.0
                                                * (double) numGoodDateOfLast
                                                / (double) dateOfLastForSect.length));
                System.out.println(
                        "\n"
                                + percDoneThresh
                                + "% done in "
            ...
            if (probTypeEnum != ProbabilityModelOptions.POISSON) {
                ...
                if (probTypeEnum == ProbabilityModelOptions.U3_BPT) {
                    for (int s = 0; s < erf.getNumFaultSystemSources(); s++) {
                        ...
                        probGainForFaultSystemSource[s] =
                                getU3_ProbGainForRup(
                                        fltSysRupIndex,
                                        0.0,
                                        false,
                                        aveRecurIntervals,
                                        aveNormTimeSinceLast,
                                        currentTimeMillis,
                                        simDuration);
                    }
                } else if (probTypeEnum == ProbabilityModelOptions.WG02_BPT) {
                    ...
                    for (int s = 0; s < erf.getNumFaultSystemSources(); s++) {
                        int fltSysRupIndex = erf.getFltSysRupIndexForSource(s);
                        probGainForFaultSystemSource[s] =
                                getWG02_ProbGainForRup(
                                        fltSysRupIndex, false, currentTimeMillis, simDuration);
                    }
                }
                ...
                for (int n = 0; n < erf.getTotNumRupsFromFaultSystem(); n++) {
                    ...
                    // test correction:
                    //					double gainCorr =
                    // gainCorrFunc.getClosestY(erf.getNthRupture(n).getMag());
                    ...
                    if (minRupProbGainArray[n] > probGain) minRupProbGainArray[n] = probGain;
                    if (maxRupProbGainArray[n] < probGain) maxRupProbGainArray[n] = probGain;
                }
            ...
            //	}
            //	DefaultXY_DataSet firstRupProbGainVsMag = new
            // DefaultXY_DataSet(temppMagArray,aveRupProbGainArray);
            ...
            //	ArrayList<PlotCurveCharacterstics> plotCharsAveGain = new
            // ArrayList<PlotCurveCharacterstics>();
            //	plotCharsAveGain.add(new PlotCurveCharacterstics(PlotSymbol.CROSS, 4f, Color.BLUE));
            //	GraphWindow graphFirstRupProbGainVsMag = new GraphWindow(temppFuncs, "First Rup Prob
            // Gain vs Mag; "+plotLabelString, plotCharsAveGain);
            ...
            // ("+(timeToNextInYrs+timeOfNextInYrs)+" yrs)");
            ...
            if (srcIndex < erf.getNumFaultSystemSources()) { // ignore other sources
                ...
                // compute and save the normalize recurrence interval if all sections had date of
                // last
                if (aveNormTimeSinceLast) { // average time since last
                    double aveNormYearsSinceLast =
                            getAveNormTimeSinceLastEventWhereKnown(
                                    fltSystRupIndex, eventTimeMillis);
                    if (allSectionsHadDateOfLast) {
                        normalizedRupRecurIntervals.add(aveNormYearsSinceLast);
                        if (numAperValues > 0)
                            normalizedRupRecurIntervalsMagDepList
                                    .get(getAperIndexForRupMag(rupMag))
                                    .add(aveNormYearsSinceLast);
                    }
                } else {
                    long aveDateOfLastMillis =
                            getAveDateOfLastEventWhereKnown(fltSystRupIndex, eventTimeMillis);
                    if (allSectionsHadDateOfLast) {
                        double timeSinceLast =
                                (eventTimeMillis - aveDateOfLastMillis) / MILLISEC_PER_YEAR;
                        double normRI =
                                timeSinceLast / aveCondRecurIntervalForFltSysRups[fltSystRupIndex];
                ...
                try {
                    eventFileWriter.write(
                            nthRup
                                    ...
                                    + eventTimeMillis
                                    + "\t"
                                    + normalizedRupRecurIntervals.get(
                                            normalizedRupRecurIntervals.size() - 1)
                                    + "\n");
                } catch (IOException e1) {
                    e1.printStackTrace();
                }
                ...
                HistogramFunction sumRI_AlongHist =
                        new HistogramFunction(
                                normRI_AlongStrike.getMinX(),
                                normRI_AlongStrike.getMaxX(),
                                normRI_AlongStrike.getNumX());
                HistogramFunction numRI_AlongHist =
                        new HistogramFunction(
                                normRI_AlongStrike.getMinX(),
                                normRI_AlongStrike.getMaxX(),
                                normRI_AlongStrike.getNumX());
                int[] sectID_Array =
                        sectIndexArrayForSrcList.get(erf.getSrcIndexForFltSysRup(fltSystRupIndex));
                ...
                for (int sect : sectID_Array) {
                    ...
                    if (timeOfLastMillis != Long.MIN_VALUE) {
                        double normYrsSinceLast =
                                ((eventTimeMillis - timeOfLastMillis) / MILLISEC_PER_YEAR)
                                        * longTermPartRateForSectArray[sect];
                        ...
                        ;
                        if (numAperValues > 0)
                            normalizedSectRecurIntervalsMagDepList
                                    .get(getAperIndexForRupMag(rupMag))
                                    .add(normYrsSinceLast);

                        double normDistAlong =
                                ((double) ithSectInRup + 0.5) / (double) numSectInRup;
                ...
                if (numSectInRup > 10) {
                    for (int i = 0; i < sumRI_AlongHist.size(); i++) {
                        ...
                        if (num > 0) {
                            normRI_AlongStrike.set(
                                    sumRI_AlongHist.getX(i), sumRI_AlongHist.getY(i) / num, 1.0);
                        }
                    }
                }
                ...
                if (numYrs < 11000 && numYrs > 1000) {
                    ...
                    if (firstEvent) {
                        for (int s = 0; s < fltSysRupSet.getNumSections(); s++) {
                            ...
                            if (sectData.getParentSectionName().contains("San Andreas")) {
                                ArbitrarilyDiscretizedFunc newFunc =
                                        new ArbitrarilyDiscretizedFunc();
                                newFunc.set(
                                        sectData.getFaultTrace().first().getLatitude(),
                                        tenPercentRI);
                                newFunc.set(
                                        sectData.getFaultTrace().last().getLatitude(),
                                        tenPercentRI);
                                safEventFuncs.add(newFunc);
                                safPlotChars4.add(
                                        new PlotCurveCharacterstics(
                                                PlotLineType.SOLID, 1, Color.BLACK));
                            }
                        }
                    }
                    ...
                    for (int id : sectID_Array) {
                        if (fltSysRupSet
                                .getFaultSectionData(id)
                                .getParentSectionName()
                                .contains("San Andreas")) safSections.add(id);
                    }
                    if (safSections.size() > 0) {
                        double[] lats =
                                new double[2 * safSections.size()]; // one for each end of the fault
                        // section
                        ArrayList<Double> shortSectRI_Lats = new ArrayList<Double>();
                        for (int i = 0; i < safSections.size(); i++) {
                            lats[2 * i] =
                                    fltSysRupSet
                                            .getFaultSectionData(safSections.get(i))
                                            .getFaultTrace()
                                            .first()
                                            .getLatitude();
                            lats[2 * i + 1] =
                            ...
                            if (timeOfLastMillis != Long.MIN_VALUE) {
                                double normYrsSinceLast =
                                        ((eventTimeMillis - timeOfLastMillis) / MILLISEC_PER_YEAR)
                                                * longTermPartRateForSectArray[safSections.get(i)];
                        ...
                        safPlotChars4.add(
                                new PlotCurveCharacterstics(PlotLineType.SOLID, 1, Color.GRAY));

                        //						double mag = magOfNthRups[nthRup];
                        //						if(mag<6.5)
                        //							safPlotChars4.add(new PlotCurveCharacterstics(PlotLineType.SOLID,
                        // 1, Color.BLUE));
                        //						else if(mag<7)
                        //							safPlotChars4.add(new PlotCurveCharacterstics(PlotLineType.SOLID,
                        // 1, Color.GREEN));
                ...
                for (int sect : sectIndexArrayForSrcList.get(srcIndex)) {
                    ...
                    double mag = magOfNthRups[nthRup];
                    if (mag > 6 && mag < 6.7) obsSectRateArrayM6pt05to6pt65[sect] += 1;
                    else if (mag > 7.9 && mag < 8.3) obsSectRateArrayM7pt95to8pt25[sect] += 1;
                }
            }
            ...
            // currentTimeMillis="+currentTimeMillis+"; timeToNextInYrs="+timeToNextInYrs);
        ...
        DefaultXY_DataSet aveRupProbGainVsMag =
                new DefaultXY_DataSet(tempMagArray, aveRupProbGainArray);
        DefaultXY_DataSet minRupProbGainVsMag =
                new DefaultXY_DataSet(tempMagArray, minRupProbGainArray);
        DefaultXY_DataSet maxRupProbGainVsMag =
                new DefaultXY_DataSet(tempMagArray, maxRupProbGainArray);
        aveRupProbGainVsMag.setName("Ave Rup Prob Gain vs Mag");
        double meanProbGain = 0;
        for (double val : aveRupProbGainArray) meanProbGain += val;
        ...
        ArrayList<PlotCurveCharacterstics> plotCharsAveGain =
                new ArrayList<PlotCurveCharacterstics>();
        plotCharsAveGain.add(new PlotCurveCharacterstics(PlotSymbol.CROSS, 4f, Color.BLUE));
        ...
        GraphWindow graphAveRupProbGainVsMag =
                new GraphWindow(
                        aveRupProbGainVsMagFuncs,
                        "Ave Rup Prob Gain vs Mag; " + plotLabelString,
                        plotCharsAveGain);
        ...
        try {
            ...
            gain_fr.write(
                    "nthRupIndex\taveRupGain\tminRupGain\tmaxRupGain\trupMag\trupLongTermRate\trupCondRI\trupName\n");
            for (int i = 0; i < aveRupProbGainArray.length; i++) {
                gain_fr.write(
                        i
                                + "\t"
                                + aveRupProbGainArray[i]
                                + "\t"
                                + minRupProbGainArray[i]
                                + "\t"
                                + maxRupProbGainArray[i]
                                + "\t"
                                + magOfNthRups[i]
                                + "\t"
                                + longTermRateOfNthRups[i]
                                + "\t"
                                + aveCondRecurIntervalForFltSysRups[
                                        erf.getFltSysRupIndexForNthRup(i)]
                                + "\t"
        ...
        for (int i = 0; i < aveRupProbGainArray.length; i++) {
            aveGainVsMagHist.add(
                    magOfNthRups[i], aveRupProbGainArray[i] * longTermRateOfNthRups[i]);
        ...
        GraphWindow graphAveGainVsMagHist =
                new GraphWindow(aveGainVsMagHist, "Ave Rup Gain vs Mag; " + plotLabelString);
        ...
        ArrayList<EvenlyDiscretizedFunc> funcList =
                ProbModelsPlottingUtils.getNormRI_DistributionWithFits(
                        normalizedRupRecurIntervals, aper);
        GraphWindow grapha_a =
                ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                        funcList, "Normalized Rupture RIs; " + plotLabelString);
        //		GraphWindow grapha_a =
        // General_EQSIM_Tools.plotNormRI_Distribution(normalizedRupRecurIntervals, "Normalized
        // Rupture RIs; "+plotLabelString, aperiodicity);
        ...
        if (numAperValues > 1) {
            for (int i = 0; i < numAperValues; i++) {
                ArrayList<EvenlyDiscretizedFunc> funcListMagDep =
                        ProbModelsPlottingUtils.getNormRI_DistributionWithFits(
                                normalizedRupRecurIntervalsMagDepList.get(i), aperValues[i]);
                String label = getMagDepAperInfoString(i);
                GraphWindow graphaMagDep =
                        ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                                funcListMagDep, "Norm Rup RIs; " + label + "; " + plotLabelString);
                ...
                // save plot now
                try {
                    graphaMagDep.saveAsPDF(
                            dirNameForSavingFiles + "/normRupRecurIntsForMagRange" + i + ".pdf");
                    graphaMagDep.saveAsTXT(
                            dirNameForSavingFiles + "/normRupRecurIntsForMagRange" + i + ".txt");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        ...
        ArrayList<EvenlyDiscretizedFunc> funcList2 =
                ProbModelsPlottingUtils.getNormRI_DistributionWithFits(
                        normalizedSectRecurIntervals, aper);
        GraphWindow graph2_b =
                ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                        funcList2, "Normalized Section RIs; " + plotLabelString);
        infoString += "\n\nSect " + funcList2.get(0).getName() + ":";
        infoString += "\n" + funcList2.get(0).getInfo();

        // make normalized sect recurrence interval plot for test section
        String sectName =
                erf.getSolution().getRupSet().getFaultSectionData(testSectionIndex).getName();
        ArrayList<EvenlyDiscretizedFunc> funcListTestSect =
                ProbModelsPlottingUtils.getNormRI_DistributionWithFits(
                        normalizedSectRecurIntervalsForTestSect, aper);
        GraphWindow graphTestSect =
                ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                        funcListTestSect, "Normalized Section RIs for " + sectName);
        ...
        if (numAperValues > 1) {
            for (int i = 0; i < numAperValues; i++) {
                ArrayList<EvenlyDiscretizedFunc> funcListMagDep =
                        ProbModelsPlottingUtils.getNormRI_DistributionWithFits(
                                normalizedSectRecurIntervalsMagDepList.get(i), aperValues[i]);
                String label = getMagDepAperInfoString(i);
                GraphWindow graphaMagDep =
                        ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                                funcListMagDep, "Norm Sect RIs; " + label + "; " + plotLabelString);
                ...
                try {
                    graphaMagDep.saveAsPDF(
                            dirNameForSavingFiles + "/normSectRecurIntsForMagRange" + i + ".pdf");
                    graphaMagDep.saveAsTXT(
                            dirNameForSavingFiles + "/normSectRecurIntsForMagRange" + i + ".txt");
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
        ...
        if (probTypeEnum == ProbabilityModelOptions.U3_BPT && doThis) {
            ...
            simNormTimeSinceLastHist.scale(
                    1.0
                            / (simNormTimeSinceLastHist.calcSumOfY_Vals()
                                    * simNormTimeSinceLastHist
                                            .getDelta())); // makes it a density function
            ArrayList<EvenlyDiscretizedFunc> funcListForSimNormTimeSinceLastHist =
                    ProbModelsPlottingUtils.addBPT_Fit(simNormTimeSinceLastHist);
            simNormTimeSinceLastHist.setName("simNormTimeSinceLastHist");
            //			simNormTimeSinceLastHist.setInfo("Dist of normalized time since last for each
            // rupture at all event times\nMean =
            // "+simNormTimeSinceLastHist.computeMean()+"\nnumObs="+numObs);
            simNormTimeSinceLastHist.setInfo(
                    "Dist of normalized time since last for each rupture at all event times\nMean = "
                            + simNormTimeSinceLastHist.computeMean());
            GraphWindow graphSimNormTimeSinceLastHist =
                    ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                            funcListForSimNormTimeSinceLastHist,
                            "simNormTimeSinceLastHist; " + plotLabelString);

            //			simNormTimeSinceLastForMagBelow7_Hist
            //			numObs = (int)Math.round(simNormTimeSinceLastForMagBelow7_Hist.calcSumOfY_Vals());
            simNormTimeSinceLastForMagBelow7_Hist.scale(
                    1.0
                            / (simNormTimeSinceLastForMagBelow7_Hist.calcSumOfY_Vals()
                                    * simNormTimeSinceLastForMagBelow7_Hist
                                            .getDelta())); // makes it a density function
            ArrayList<EvenlyDiscretizedFunc> funcListForSimNormTimeSinceLastHistForSmall =
                    ProbModelsPlottingUtils.addBPT_Fit(simNormTimeSinceLastForMagBelow7_Hist);
            simNormTimeSinceLastForMagBelow7_Hist.setName("simNormTimeSinceLastForMagBelow7_Hist");
            //			simNormTimeSinceLastForMagBelow7_Hist.setInfo("Dist of normalized time since last
            // for each rupture at all event times from M<=7\nMean =
            // "+simNormTimeSinceLastForMagBelow7_Hist.computeMean()+"\nnumObs="+numObs);
            simNormTimeSinceLastForMagBelow7_Hist.setInfo(
                    "Dist of normalized time since last for each rupture at all event times from M<=7\nMean = "
                            + simNormTimeSinceLastForMagBelow7_Hist.computeMean());
            GraphWindow graphSimNormTimeSinceLastForMagBelow7_Hist =
                    ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                            funcListForSimNormTimeSinceLastHistForSmall,
                            "simNormTimeSinceLastForMagBelow7_Hist; " + plotLabelString);

            simNormTimeSinceLastForMagAbove7_Hist.scale(
                    1.0
                            / (simNormTimeSinceLastForMagAbove7_Hist.calcSumOfY_Vals()
                                    * simNormTimeSinceLastForMagAbove7_Hist
                                            .getDelta())); // makes it a density function
            ArrayList<EvenlyDiscretizedFunc> funcListForSimNormTimeSinceLastHistForLarge =
                    ProbModelsPlottingUtils.addBPT_Fit(simNormTimeSinceLastForMagAbove7_Hist);
            simNormTimeSinceLastForMagAbove7_Hist.setName("simNormTimeSinceLastForMagAbove7_Hist");
            simNormTimeSinceLastForMagAbove7_Hist.setInfo(
                    "Dist of normalized time since last for each rupture at all event times from M>7\nMean = "
                            + simNormTimeSinceLastForMagAbove7_Hist.computeMean());
            GraphWindow graphSimNormTimeSinceLastForMagAbove7_Hist =
                    ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                            funcListForSimNormTimeSinceLastHistForLarge,
                            "simNormTimeSinceLastForMagAbove7_Hist; " + plotLabelString);

            //			numObs = (int)Math.round(simProbGainHist.calcSumOfY_Vals());
            simProbGainHist.scale(
                    1.0
                            / (simProbGainHist.calcSumOfY_Vals()
                                    * simProbGainHist.getDelta())); // makes it a density function
            simProbGainHist.setName("simProbGainHist");
            //			simProbGainHist.setInfo("Dist of gains for each rupture at all event times
            // (simProbGainHist)\nMean = "+simProbGainHist.computeMean()+"\nnumObs="+numObs);
            simProbGainHist.setInfo(
                    "Dist of gains for each rupture at all event times (simProbGainHist)\nMean = "
                            + simProbGainHist.computeMean());
            ArrayList<EvenlyDiscretizedFunc> funcListForSimProbGainHist =
                    new ArrayList<EvenlyDiscretizedFunc>();
            funcListForSimProbGainHist.add(simProbGainHist);
            simProbGainForMagBelow7_Hist.scale(
                    1.0
                            / (simProbGainForMagBelow7_Hist.calcSumOfY_Vals()
                                    * simProbGainForMagBelow7_Hist
                                            .getDelta())); // makes it a density function
            simProbGainForMagAbove7_Hist.scale(
                    1.0
                            / (simProbGainForMagAbove7_Hist.calcSumOfY_Vals()
            ...
            simProbGainForMagBelow7_Hist.setInfo(
                    "Mean = " + simProbGainForMagBelow7_Hist.computeMean());
            simProbGainForMagAbove7_Hist.setInfo(
                    "Mean = " + simProbGainForMagAbove7_Hist.computeMean());
            funcListForSimProbGainHist.add(simProbGainForMagBelow7_Hist);
            funcListForSimProbGainHist.add(simProbGainForMagAbove7_Hist);
            ArrayList<PlotCurveCharacterstics> plotCharsForSimProbGainHist =
                    new ArrayList<PlotCurveCharacterstics>();
            plotCharsForSimProbGainHist.add(
                    new PlotCurveCharacterstics(PlotLineType.HISTOGRAM, 2f, Color.RED));
            plotCharsForSimProbGainHist.add(
            ...
            GraphWindow graphSimProbGainHistWindow =
                    new GraphWindow(
                            funcListForSimProbGainHist,
                            "simProbGainHist",
                            plotCharsForSimProbGainHist);
            ...
            try {
                graphSimNormTimeSinceLastHist.saveAsPDF(
                        dirNameForSavingFiles + "/simNormTimeSinceLastHist.pdf");
                graphSimProbGainHistWindow.saveAsPDF(
                        dirNameForSavingFiles + "/simProbGainHist.pdf");
                graphSimNormTimeSinceLastForMagBelow7_Hist.saveAsPDF(
                        dirNameForSavingFiles + "/simNormTimeSinceLastForMagBelow7_Hist.pdf");
                graphSimNormTimeSinceLastForMagAbove7_Hist.saveAsPDF(
                        dirNameForSavingFiles + "/simNormTimeSinceLastForMagAbove7_Hist.pdf");
                graphSimNormTimeSinceLastHist.saveAsTXT(
                        dirNameForSavingFiles + "/simNormTimeSinceLastHist.txt");
                graphSimProbGainHistWindow.saveAsTXT(
                        dirNameForSavingFiles + "/simProbGainHist.txt");
                graphSimNormTimeSinceLastForMagBelow7_Hist.saveAsTXT(
                        dirNameForSavingFiles + "/simNormTimeSinceLastForMagBelow7_Hist.txt");
                graphSimNormTimeSinceLastForMagAbove7_Hist.saveAsTXT(
                        dirNameForSavingFiles + "/simNormTimeSinceLastForMagAbove7_Hist.txt");
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
        ...
        DefaultXY_DataSet totRateVersusTime =
                new DefaultXY_DataSet(yearsIntoSimulation, totRateAtYearsIntoSimulation);
        double meanTotRate = 0;
        double totWt = 0;
        for (int i = 0; i < totRateAtYearsIntoSimulation.size() - 1; i++) {
            double wt =
                    yearsIntoSimulation.get(i + 1)
                            - yearsIntoSimulation.get(i); // apply rate until time of next event
            meanTotRate += totRateAtYearsIntoSimulation.get(i) * wt;
            totWt += wt;
        }
        meanTotRate /= totWt;
        totRateVersusTime.setName("Total Rate vs Time");
        totRateVersusTime.setInfo(
                "Mean Total Rate = " + meanTotRate + "\nLong Term Rate = " + totalLongTermRate);
        ...
        ArrayList<PlotCurveCharacterstics> plotCharsTotRate =
                new ArrayList<PlotCurveCharacterstics>();
        ...
        GraphWindow graphTotalRateVsTime =
                new GraphWindow(
                        funcsTotRate, "Total Rate vs Time; " + plotLabelString, plotCharsTotRate);
        ...
        String infoString2 =
                "total rate = " + (float) obsTotRate + " (ratio=" + (float) rateRatio + ")";
        ...
        infoString2 +=
                "\ntotal rate >= 6.7 = "
                        + (float) obsTotRateAbove6pt7
                        + " (ratio="
                        + (float) rateAbove6pt7_Ratio
                        + ")";
        ...
        GraphWindow graph =
                new GraphWindow(funcs, "Incremental Mag-Freq Dists; " + plotLabelString);
        ...
        DefaultXY_DataSet obsVsImposedRupRates =
                new DefaultXY_DataSet(longTermRateOfNthRups, obsRupRateArray);
        ...
        GraphWindow graph4 =
                new GraphWindow(funcs4, "Obs vs Imposed Rup Rates; " + plotLabelString, plotChars4);
        ...
        // plot SAF events
        GraphWindow graph9 =
                new GraphWindow(safEventFuncs, "SAF events; " + plotLabelString, safPlotChars4);
        ...
        DefaultXY_DataSet obsVsImposedSectSlipRates =
                new DefaultXY_DataSet(longTermSlipRateForSectArray, obsSectSlipRateArray);
        ...
        GraphWindow graphSR =
                new GraphWindow(
                        funcsSR,
                        "Obs vs Imposed Section Slip Rates; " + plotLabelString,
                        plotCharsSR);
        ...
        DefaultXY_DataSet obsVsImposedSectRates =
                new DefaultXY_DataSet(longTermPartRateForSectArray, obsSectRateArray);
        ...
        GraphWindow graph2 =
                new GraphWindow(
                        funcs2, "Obs vs Imposed Section Rates; " + probTypeString, plotChars);
        ...
        try {
            eventRates_fr =
                    new FileWriter(dirNameForSavingFiles + "/obsVsImposedSectionPartRates.txt");
            eventRates_fr.write(
                    "sectID\timposedRate\tsimulatedRate\tsimOverImpRateRatio\thasDateOfLast\tsectName\n");
            for (int i = 0; i < fltSysRupSet.getNumSections(); i++) {
                ...
                if (fltData.getDateOfLastEvent() != Long.MIN_VALUE) hasDateOfLast = true;
                eventRates_fr.write(
                        fltData.getSectionId()
                                + "\t"
                                + longTermPartRateForSectArray[i]
                                + "\t"
                                + obsSectRateArray[i]
                                + "\t"
                                + ratio
        ...
        EvenlyDiscretizedFunc alongFunc2pt5 =
                normRI_AlongStrike.getInterpolatedFractileCurve(0.025);
        EvenlyDiscretizedFunc alongFunc97pt5 =
                normRI_AlongStrike.getInterpolatedFractileCurve(0.975);
        ...
        GraphWindow graph8 =
                new GraphWindow(
                        funcs8,
                        "Normalized RI vs Normalized Dist Along Strike; " + probTypeString,
                        plotChars8);
        ...
        for (int s = 0; s < numSect; s++) {
            ...
            for (int r : fltSysRupSet.getRupturesForSection(s)) {
                ...
                if (mag > 6 && mag < 6.7) partRateMlow += fltSysSolution.getRateForRup(r);
                else if (mag > 7.9 && mag < 8.3) partRateMhigh = fltSysSolution.getRateForRup(r);
            }
        ...
        for (int s = 0; s < numSect; s++) {
            if (predSectRateArrayM6pt05to6pt65[s]
                    >= 10.0 / numYears) { // only keep where 10 should have occurred
                obs_pred_ratioForSections.set(
                        predSectRateArrayM6pt05to6pt65[s],
                        obsSectRateArrayM6pt05to6pt65[s] / predSectRateArrayM6pt05to6pt65[s]);
            }
        }
        ...
        GraphWindow graph3 =
                new GraphWindow(
                        funcs3,
                        "Obs/imposed vs Imposed Section Rates for M 6.0 to 6.7; " + plotLabelString,
                        plotChars2);
        ...
        try {
            ...
            graphTestSect.saveAsPDF(
                    dirNameForSavingFiles + "/normalizedRecurIntervalsForTestSect.pdf");
            graphTestSect.saveAsTXT(
                    dirNameForSavingFiles + "/normalizedRecurIntervalsForTestSect.txt");
            ...
            graph3.saveAsPDF(
                    dirNameForSavingFiles + "/obsOverImposedVsImposedSectionPartRatesM6to6pt7.pdf");
            ...
            // data:
            FileWriter fr =
                    new FileWriter(dirNameForSavingFiles + "/normalizedRupRecurIntervals.txt");
            for (double val : normalizedRupRecurIntervals) fr.write(val + "\n");
            fr.close();

            fr = new FileWriter(dirNameForSavingFiles + "/normalizedSectRecurIntervals.txt");
            for (double val : normalizedSectRecurIntervals) fr.write(val + "\n");
            fr.close();

            AbstractDiscretizedFunc.writeSimpleFuncFile(
                    targetMFD, dirNameForSavingFiles + "/targetMFD.txt");
            AbstractDiscretizedFunc.writeSimpleFuncFile(
                    obsMFD, dirNameForSavingFiles + "/simulatedMFD.txt");
            AbstractDiscretizedFunc.writeSimpleFuncFile(
                    targetMFD.getCumRateDistWithOffset(),
                    dirNameForSavingFiles + "/targetCumMFD.txt");
            AbstractDiscretizedFunc.writeSimpleFuncFile(
                    obsMFD.getCumRateDistWithOffset(),
                    dirNameForSavingFiles + "/simulatedCumMFD.txt");
     ...
     *
     * <p>MFD and moment rate comparisons are not yet implemented correctly
     */
    public void testER_SimulationOnParentSection(
            String inputDateOfLastFileName,
            String outputDateOfLastFileName,
            FaultSystemSolutionERF erf,
            double numYears,
        ...
        String dirNameForSavingFiles =
                "Psect_" + parentSectIndex + "_" + probTypeString + "_" + tempDur + "kyr";
        if (probTypeEnum != ProbabilityModelOptions.POISSON) {
            dirNameForSavingFiles += "_" + aperString;
            dirNameForSavingFiles += "_" + typeCalcForU3_Probs;
        }
        ...
        List<Integer> fltSysRupIndicesForParentList =
                fltSysRupSet.getRupturesForParentSection(parentSectIndex);
        ...
        for (ProbEqkSource src : erf) {
            for (ProbEqkRupture rup : src) {
                ...
                if (fltSysRupIndicesForParentList.contains(fltSysRupIndex))
                    longTermRateOfNthRups[nthRup] = rate;
                else longTermRateOfNthRups[nthRup] = 0;
                magOfNthRups[nthRup] = rup.getMag();
                totalRate += longTermRateOfNthRups[nthRup];
        ...
        while (currentYear < numYears + origStartYear) {
            ...
            else timeToNextInYrs = simDuration * 10; // just make the next text fail
            ...
            if (timeToNextInYrs <= simDuration) { // only keep if withing simDuration
                ...
                if (srcIndex < erf.getNumFaultSystemSources()) { // ignore other sources
                    ...
                    // compute and save the normalize recurrence interval if all sections had date
                    // of last
                    if (aveNormTimeSinceLast) { // average time since last
                        double aveNormYearsSinceLast =
                                getAveNormTimeSinceLastEventWhereKnown(
                        ...
                        if (allSectionsHadDateOfLast) {
                            ...
                            double normRI =
                                    timeSinceLast
                                            / aveCondRecurIntervalForFltSysRups[fltSystRupIndex];
                            normalizedRupRecurIntervals.add(normRI);
                            if (numAperValues > 0)
                                normalizedRupRecurIntervalsMagDepList
                                        .get(getAperIndexForRupMag(rupMag))
                    ...
                    // save normalized fault section recurrence intervals & RI along strike
                    int[] sectID_Array =
                            sectIndexArrayForSrcList.get(
                                    erf.getSrcIndexForFltSysRup(fltSystRupIndex));
                    int numSectInRup = sectID_Array.length;
                    double slips[];
                    if (fltSysRupSet instanceof InversionFaultSystemRupSet) {
                        slips =
                        ...
                        double area =
                                fltSysRupSet.getAreaForRup(erf.getFltSysRupIndexForNthRup(nthRup));
                        double aveSlip =
                                FaultMomentCalc.getSlip(area, MagUtils.magToMoment(rupMag));
                        slips = new double[numSectInRup];
                        for (int i = 0; i < slips.length; i++) slips[i] = aveSlip;
                    }
                    // obsSectSlipRateArray
        ...
        try {
            gain_fr = new FileWriter(dirNameForSavingFiles + "/aveRupGainData.txt");
            gain_fr.write(
                    "nthRupIndex\taveRupGain\tminRupGain\tmaxRupGain\trupMag\trupLongTermRate\tobsRupRate\texpNumRups\trupCondRI\trupName\n");
            for (int i = 0; i < aveRupProbGainArray.length; i++) {
                if (longTermRateOfNthRups[i] > 0)
                    gain_fr.write(
                            i
                                    ...
                                    + obsRupRateArray[i]
                                    + "\t"
                                    + numYears * longTermRateOfNthRups[i]
                                    + "\t"
                                    + aveCondRecurIntervalForFltSysRups[
                                            erf.getFltSysRupIndexForNthRup(i)]
                                    + "\t"
    ...
    /**
     * This simulates a number of next x-year catalogs (e.g., to check for biases at long
     * durations).
     *
     * <p>The start time and duration are obtained from the ERF, and we assume updateForecast() has
     * been called.
     *
     * <p>Note that the predicted sections rates are expected to be biased high where second events
     * are likely (the simulation includes these, but the targets do not). These cases should be
     * closer to the long-term rate to the extend that expected number (longTermSecRate*Duration) is
     * high.
     *
     * @param erf
     * @param dirNameSuffix
     ...
     * @param numCatalogs
     */
    public void testER_NextXyrSimulation(
            File resultsDir,
            String inputDateOfLastFileName,
            int numCatalogs,
            boolean makePlots,
            Long randomSeed)
            throws IOException {
        testER_NextXyrSimulation(
                resultsDir,
                inputDateOfLastFileName,
                numCatalogs,
                makePlots,
                randomSeed,
     ...
     * durations).
     *
     * <p>The start time is obtained from the ERF, and we assume updateForecast() has been called.
     *
     * <p>Note that the predicted sections rates are expected to be biased high where second events
     * are likely (the simulation includes these, but the targets do not). These cases should be
     * closer to the long-term rate to the extend that expected number (longTermSecRate*Duration) is
    ...
    public void testER_NextXyrSimulation(
        ...
        ArrayList<U3PaleoRateConstraint> paleoConstraints =
                UCERF3_PaleoRateConstraintFetcher.getConstraints(
                        fltSysRupSet.getFaultSectionDataList());
        ...
        for (U3PaleoRateConstraint constr : paleoConstraints) {
            ...
            info_fr.write(
                    "\t"
                            + constr.getPaleoSiteName()
                            + "\t"
                            + constr.getSectionIndex()
                            + "\t"
                            + constr.getFaultSectionName()
                            + "\n");
        }
        ...
        if (randomSeed == null) {
            ...
            info_fr.write(
                    "\nRandom Seed: "
                            + randomSeed
                            + " (from System.currentTimeMillis() since input null)\n");
        } else {
        ...
        IntegerPDF_FunctionSampler nthRupRandomSampler =
                new IntegerPDF_FunctionSampler(erf.getTotNumRups());
        double[] targetRateOfNthRups =
                new double[erf.getTotNumRups()]; // this will include any aftershock reductions
        double[] targetRateOfSection =
                new double[numSections]; // this will include any aftershock reductions
        double[] magOfNthRups = new double[erf.getTotNumRups()];
        int nthRup = 0;
        double maxRupProb = 0;
        for (ProbEqkSource src : erf) {
            for (ProbEqkRupture rup : src) {
                if (rup.getProbability() > maxRupProb) maxRupProb = rup.getProbability();
                double rate = rup.getMeanAnnualRate(erf.getTimeSpan().getDuration());
        ...
        System.out.println(
                "totalRate long term = " + totalTargetRate + "\nmaxRupProb = " + maxRupProb);
        info_fr.write(
        ...
        for (nthCatalog = 0; nthCatalog < numCatalogs; nthCatalog++) {
            ...
            // write progress
            if (progressBar != null) progressBar.updateProgress(nthCatalog, numCatalogs);
            ...
            while (currentYear <= origStartYear + forecastDurationYrs) {
                ...
                if (probTypeEnum != ProbabilityModelOptions.POISSON) {
                    ...
                    // now update totalRate and ruptureSampler (for all rups since start time
                    // changed)
                    for (int n = 0; n < erf.getTotNumRupsFromFaultSystem(); n++) {
                        double probGain = probGainForFaultSystemSource[erf.getSrcIndexForNthRup(n)];
                        ...
                        //						if(probTypeEnum == ProbabilityModelOptions.U3_BPT) {
                        //							double prob =
                        // probGain*longTermRateOfFltSysRup[erf.getFltSysRupIndexForNthRup(n)]*thisDuration;	// this will crash if background seismicity included!  should create: longTermRateOfNthRups[n];
                        //							newRate = -Math.log(1.0-prob)/thisDuration;
                        //						}
                        //						else {
                        //							newRate =
                        // probGain*longTermRateOfFltSysRup[erf.getFltSysRupIndexForNthRup(n)];	//
                        // this will crash if background seismicity included!  should create:
                        // longTermRateOfNthRups[n];				//
                        //						}

                        double newRate =
                                probGain
                                        * longTermRateOfFltSysRup[
                                                erf.getFltSysRupIndexForNthRup(
                                                        n)]; // this will crash if background
                        // seismicity included!  should create:
                        // longTermRateOfNthRups[n];				//
                        nthRupRandomSampler.set(n, newRate);
                    }
                ...
                if (currentYear <= origStartYear + forecastDurationYrs) {
                    ...
                    if (srcIndex < erf.getNumFaultSystemSources()) { // ignore other sources
                        ...
                        // compute and save the normalize recurrence interval if all sections had
                        // date of last
                        if (aveNormTimeSinceLast) { // average time since last
                            normRI =
                                    getAveNormTimeSinceLastEventWhereKnown(
                                            fltSystRupIndex, currentTimeMillis);
                            if (allSectionsHadDateOfLast) {
                                normalizedRupRecurIntervals.add(normRI);
                            }
                        } else {
                            long aveDateOfLastMillis =
                                    getAveDateOfLastEventWhereKnown(
                                            fltSystRupIndex, currentTimeMillis);
                            if (allSectionsHadDateOfLast) {
                                double timeSinceLast =
                                        (currentTimeMillis - aveDateOfLastMillis)
                                                / MILLISEC_PER_YEAR;
                                normRI =
                                        timeSinceLast
                                                / aveCondRecurIntervalForFltSysRups[
        ...
        if (makePlots) {
            ...
            //			}
            //			DefaultXY_DataSet obsVsImposedRupRates = new
            // DefaultXY_DataSet(targetRateOfNthRups,obsRupRateArray);
            //			obsVsImposedRupRates.setName("Simulated vs Imposed Rup Rates");
            //			DefaultXY_DataSet perfectAgreementFunc4 = new DefaultXY_DataSet();
            //			perfectAgreementFunc4.set(1e-5,1e-5);
            //			perfectAgreementFunc4.set(0.05,0.05);
            ...
            //			funcs4.add(obsVsImposedRupRates);
            //			funcs4.add(perfectAgreementFunc4);
            //			ArrayList<PlotCurveCharacterstics> plotChars4 = new
            // ArrayList<PlotCurveCharacterstics>();
            //			plotChars4.add(new PlotCurveCharacterstics(PlotSymbol.CROSS, 4f, Color.BLUE));
            //			plotChars4.add(new PlotCurveCharacterstics(PlotLineType.SOLID, 2f, Color.RED));
            //			GraphWindow rupRates_graph = new GraphWindow(funcs4, "Obs vs Imposed Rup Rates;
            // "+plotLabelString, plotChars4);
            ...
            DefaultXY_DataSet obsVsImposedSectRates =
                    new DefaultXY_DataSet(targetRateOfSection, obsSectRateArray);
            obsVsImposedSectRates.setName("Simulated vs Target Section Event Rates");
            double rateTrans = 1.0 / forecastDurationYrs;
            obsVsImposedSectRates.setInfo(
                    "Simulated rates should be above target for rates above ~"
                            + (float) rateTrans
                            + " due to second events in simulation");
            DefaultXY_DataSet perfectAgreementFunc = new DefaultXY_DataSet();
            perfectAgreementFunc.set(1e-5, 1e-5);
            perfectAgreementFunc.set(0.05, 0.05);
            perfectAgreementFunc.setName("Perfect agreement line");
            ...
            plotChars.add(new PlotCurveCharacterstics(PlotSymbol.CROSS, 4f, Color.BLUE));
            plotChars.add(new PlotCurveCharacterstics(PlotLineType.SOLID, 2f, Color.RED));
            GraphWindow sectRates_graph =
                    new GraphWindow(funcs2, "Obs vs Target Section Rates; ", plotChars);
            ...
            try {
                eventRates_fr =
                        new FileWriter(new File(resultsDir, "obsVsTargetSectionPartRates.txt"));
                eventRates_fr.write(
                        "sectID\tlongTermRate\ttargetRate\tsimulatedRate\tsimOverTargetRateRatio\taveSectGain\thasDateOfLast\tnormTimeSince\tsectName\n");
                for (int i = 0; i < fltSysRupSet.getNumSections(); i++) {
                    FaultSection fltData = fltSysRupSet.getFaultSectionData(i);
                    ...
                    if (hasDateOfLast)
                        normTimeSince =
                                (((double) (origStartTimeMillis - origDateOfLastForSect[i]))
                                                / MILLISEC_PER_YEAR)
                                        * longTermPartRateForSectArray[i];
                    else normTimeSince = Double.NaN;
                    eventRates_fr.write(
                            fltData.getSectionId()
                                    + "\t"
                                    + longTermPartRateForSectArray[i]
                                    + "\t"
                                    + targetRateOfSection[i]
                                    + "\t"
                                    + obsSectRateArray[i]
                                    + "\t"
                                    + ratio
                                    + "\t"
                                    + (targetRateOfSection[i] / longTermPartRateForSectArray[i])
                                    + "\t"
                                    + hasDateOfLast
                                    + "\t"
                                    + normTimeSince
            ...
            try {
                ...
                normRI_graph.saveAsTXT(
                        dirNameForSavingFiles + "/normalizedRupRecurIntervalsPlot.txt");
                mfd_graph.saveAsPDF(dirNameForSavingFiles + "/magFreqDists.pdf");
                sectRates_graph.saveAsPDF(
                        dirNameForSavingFiles + "/obsVsTargetSectionPartRates.pdf");
    ...
    private void writeSectTimeSinceLastEventToFile(String fileName, long currentTimeMillis) {
        ...
        try {
                ...
                if (dateOfLastForSect[i] != Long.MIN_VALUE) {
                    ...
                    if (timeSince < 0) {
                        if (timeSince > -MILLISEC_PER_YEAR) {
                            System.out.println(
                                    "Converting slightly negative time since last ("
                                            + timeSince
                                            + ") to zero");
                            timeSince = 0;
                        } else {
            ...
            fileWriter.close();
            int percBad =
                    (int) Math.round(100.0 * (double) numBad / (double) dateOfLastForSect.length);
            System.out.println(
                    numBad
                            + " sections out of "
                            + dateOfLastForSect.length
                            + " had no date of last event in output file ("
                            + percBad
                            + "%)");
    ...
    private GraphWindow getSectNormTimeSinceLastHistPlot(
            long currentTimeMillis, String labelPrefix) {
        ...
        for (int s = 0; s < dateOfLastForSect.length; s++) {
            ...
            if (epochOfLast != Long.MIN_VALUE) {
                double yearsSinceLast =
                        ((double) (currentTimeMillis - epochOfLast) / MILLISEC_PER_YEAR);
    ...
    /**
     * This returns an array of normalized time since last event for each section, with a value of
     * Double.NaN if it's unknown
     *
    ...
    private void readSectTimeSinceLastEventFromFile(String fileName, long currentTimeMillis) {
        ...
        try {
            ...
            System.out.println(
                    "Reading file " + fileName + "; currentTimeMillis+" + currentTimeMillis);

            BufferedReader reader =
                    new BufferedReader(UCERF3_DataUtils.getReader(dataFile.toURL()));
            //			BufferedReader reader = new
            // BufferedReader(scratch.UCERF3.utils.UCERF3_DataUtils.getReader(dataFile.getAbsolutePath()));
            int s = 0;
            String line;
            int numBad = 0;
            while ((line = reader.readLine()) != null) {
                ...
                if (s != sectIndex) throw new RuntimeException("bad index");
                s += 1;
            }
            int percBad =
                    (int) Math.round(100.0 * (double) numBad / (double) dateOfLastForSect.length);
            System.out.println(
                    numBad
                            + " sections out of "
                            + dateOfLastForSect.length
                            + " had no date of last event in input file ("
                            + percBad
                            + "%)");
    ...
    /**
     * This writes out the rupture probability gains for the 4 different averaging combinations to a
     * file, along with other info about the rupture
     *
     * @param presentTimeMillis
     * @param durationYears
     * @param fileName
     */
    public void writeRupProbGainsForDiffAveragingMethods(
            long presentTimeMillis, double durationYears, String fileName) {
     ...
     * @param durationYears
     * @param fileName
     * @param subSectIndex - only include ruptures that utilize this subsection (set as -1 for all
     *     subsections)
     */
    public void writeRupProbGainsForDiffAveragingMethods(
            long presentTimeMillis, double durationYears, String fileName, int subSectIndex) {
        ...
        if (subSectIndex != -1) rupIndexList = fltSysRupSet.getRupturesForSection(subSectIndex);

        try {
            fileWriter = new FileWriter(dataFile);
            fileWriter.write(
                    "rupIndex\tRI_NTS\tRI_TS\tRateNTS\tRateTS\tcondRI_RI\tcondRI_Rate\tlongTermRate\tmaxOverMin\tMaxMinusMin\tsigDiff\trupMag\trupAper\trupName\n");
            for (int fltSystRupIndex = 0;
                    fltSystRupIndex < numRupsInFaultSystem;
                    fltSystRupIndex++) {
                if (longTermRateOfFltSysRup[fltSystRupIndex] > 0.0) {
                    ...
                    line +=
                            "\t"
                                    + aveCondRecurIntervalForFltSysRups_type1[fltSystRupIndex]
                                    + "\t"
                                    ...
                                    + rupMag
                                    + "\t"
                                    + aperValues[getAperIndexForRupMag(rupMag)]
                                    + "\t"
                                    + name;
                    if (subSectIndex == -1) fileWriter.write(line + "\n");
                    else if (rupIndexList.contains(fltSystRupIndex)) fileWriter.write(line + "\n");
                }
            }
            fileWriter.close();
    ...
    public String getInfoAboutRupture(int fltSystRupIndex, long presentTimeMillis) {
        ...
        for (int sectIndex : sectIndicesList) {
            ...
            if (dateOfLastMillis != Long.MIN_VALUE)
                yrsSinceLast =
                        ((double) (presentTimeMillis - dateOfLastMillis)) / MILLISEC_PER_YEAR;
            info +=
                    sectIndex
                            + "\t"
                            + (1.0 / longTermPartRateForSectArray[sectIndex])
                            + "\t"
                            + longTermPartRateForSectArray[sectIndex]
                            + "\t"
                            + yrsSinceLast
                            + "\t"
                            + yrsSinceLast * longTermPartRateForSectArray[sectIndex]
                            + "\t"
                            + sectionArea[sectIndex]
                            + "\t"
                            + fltData.getName()
                            + "\n";
        }
    ...
    public String getInfoAboutRupsOnSection(int sectID, String fileName) {
        ...
        for (int rupID : rupIDsList) {
            info +=
                    rupID
                            + "\t"
                            + longTermRateOfFltSysRup[rupID]
                            + "\t"
                            + fltSysRupSet.getMagForRup(rupID)
                            + "\n";
        }
        //		File dataFile = new File(dataDir,File.separator+fileName);
        try {
    ...
    /**
     * Two segment/section example, where zeroeth rupture only uses section 0, and 1st rup uses both
     * section 0 and 1.
     *
    ...
    public static void simpleModelTest(int numSimulatedRups) {
        ...
        double[] sectYearOfLastArray = {
            -1.0 / longTermSectRateArray[0], -1.0 / longTermSectRateArray[1]
        }; // start of one sect RI since last
        ...
        //		condRI_Array[1] = 1.0/((longTermSectRateArray[0]+longTermSectRateArray[1])/2.0);	// ave
        // section RIs

        System.out.println(
                "longTermRupRateArray: "
                        + longTermRupRateArray[0]
                        + ", "
                        + longTermRupRateArray[1]);
        System.out.println(
                "longTermSectRateArray: "
                        + longTermSectRateArray[0]
                        + ", "
                        + longTermSectRateArray[1]);
        ...
        while (nthRup < numSimulatedRups) {
            ...
            aveNormTimeSinceArray[1] =
                    ((year - sectYearOfLastArray[0]) * longTermSectRateArray[0]
                                    + (year - sectYearOfLastArray[1]) * longTermSectRateArray[1])
                            / 2;
            condProbArray[0] =
                    bptRefCalc.getCondProb(
                            aveNormTimeSinceArray[0], stepDurationYears * refRI / condRI_Array[0]);
            condProbArray[1] =
                    bptRefCalc.getCondProb(
                            aveNormTimeSinceArray[1], stepDurationYears * refRI / condRI_Array[1]);
            tdRupRateArray[0] =
                    (condProbArray[0] / (stepDurationYears / condRI_Array[0]))
                            * (1.0 / rup1_RI); // (gain)*(longTermRate)
            tdRupRateArray[1] =
                    (condProbArray[1] / (stepDurationYears / condRI_Array[1]))
                            * (1.0 / rup2_RI); // (gain)*(longTermRate)
            ...
            if (timeToNextInYrs < stepDurationYears) {
                ...
                if (r == 0) {
                    ...
                    normalizedSectRecurIntervals.add(
                            (year - sectYearOfLastArray[0]) * longTermSectRateArray[0]);
        ...
        System.out.println(
                "simulated rate 0th rup = "
                        + (num0thRups / year)
                        + "\t("
                        + ((num0thRups / year) / longTermRupRateArray[0])
                        + ")");
        System.out.println("expected rate 0th rup = " + longTermRupRateArray[0]);
        System.out.println(
                "simulated rate 1st rup = "
                        + ((numSimulatedRups - num0thRups) / year)
                        + "\t("
                        + (((numSimulatedRups - num0thRups) / year) / longTermRupRateArray[1])
                        + ")");
        ...
        GraphWindow grapha_a =
                ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                        funcList, "Normalized Rupture RIs; " + "simple problem");
        String infoString = "";
        infoString += "\nRup " + funcList.get(0).getName() + ":";
        infoString += "\n" + funcList.get(0).getInfo();

        ArrayList<EvenlyDiscretizedFunc> funcListSect =
                ProbModelsPlottingUtils.getNormRI_DistributionWithFits(
                        normalizedSectRecurIntervals, aper);
        GraphWindow grapha_b =
                ProbModelsPlottingUtils.plotNormRI_DistributionWithFits(
                        funcListSect, "Normalized Section RIs; " + "simple problem");
    ...
    /**
     * This plot a histogram of the ratio of aveRate vs aveRI conditional RIs, over all ruptures,
     * where each is weighted by their long-term rate.
     */
    public void plotRatioHistOfRupCondProbs() {
        ...
        for (int r = 0; r < aveCondRecurIntervalForFltSysRups_type1.length; r++) {
            double ratio =
                    aveCondRecurIntervalForFltSysRups_type2[r]
                            / aveCondRecurIntervalForFltSysRups_type1[r];
            if (ratio < hist.getMaxX() + hist.getDelta() / 2)
                hist.add(ratio, longTermRateOfFltSysRup[r]);
            else System.out.println(ratio);
        }
        hist.setInfo("mean=" + (float) hist.computeMean());
        GraphWindow graphMagHist =
                new GraphWindow(hist, "Ratio of aveRate to aveRI Cond RI values");
    }
    ...
    public void setFltSystemRupOccurranceTime(int fltSysRupIndex, Long epoch) {
        // System.out.println("setFltSystemRupOccurranceTime was called for
        // fltSysRupIndex="+fltSysRupIndex+";  epoch="+epoch);
    ...
    public static void main(String[] args) {
        ...
        ////		BPTAveragingTypeOptions aveType =
        ...
        ////		testCalc.testER_Next50yrSimulation(erf,
        // "SimpleFaultTest_Next50yrSimBPT_100yrTestGainFix", null, 5000);
        //
        //		System.exit(0);
        ...
        //		erf.updateForecast();
        //		ProbabilityModelsCalc testCalc = new ProbabilityModelsCalc(erf);
        //		testCalc.testER_Simulation(null, null, erf,100000000d,
        // "SimpleFaultTestRun1_Dec18_Alt9");
        ////		testCalc.testER_Next50yrSimulation(erf,
        // "SimpleFaultTest_Next50yrSimBPT_100yrTestGainFix", null, 5000);

        String fileName =
                "dev/scratch/UCERF3/data/scratch/InversionSolutions/2013_05_10-ucerf3p3-production-10runs_COMPOUND_SOL_FM3_1_MEAN_BRANCH_AVG_SOL.zip";
        ...
        erf.getParameter(ProbabilityModelParam.NAME).setValue(ProbabilityModelOptions.U3_BPT);
        erf.getParameter(MagDependentAperiodicityParam.NAME)
                .setValue(MagDependentAperiodicityOptions.MID_VALUES);
        //
        ...
        //			index+=1;
        //		}
        //		System.out.println("Pitman Canyon section:
        // "+minIndex+"\t"+erf.getSolution().getRupSet().getFaultSectionData(minIndex).getName());
        //		System.exit(-1);

        // TODO This could be obtained from the ERF, but the ERF would have to use the
        // ProbabilityModelsCalc constructor that takes the ERF
        ProbabilityModelsCalc testCalc = new ProbabilityModelsCalc(erf);
        ...
        //		testCalc.testER_SimulationOnParentSection(timeSinceLastFileName, null, erf,60000000d,
        // "CerroP",172);

        // Biggest prob diff between viable approaches on Imperial section 0
        //		System.out.println(testCalc.getInfoAboutRupture(198219,
        // erf.getTimeSpan().getStartTimeInMillis()));

        //		// Biggest prob diff between viable approaches on Mojave sections
        //		System.out.println(testCalc.getInfoAboutRupture(884,
        // erf.getTimeSpan().getStartTimeInMillis()));

        // Biggest prob diff between viable approaches on Owens Valley sections
        //		System.out.println(testCalc.getInfoAboutRupture(249574,
        // erf.getTimeSpan().getStartTimeInMillis()));

        // Biggest prob diff between RI_TS and RI_NTS on "San Andreas (Mojave S), Subsection 3"	1840
        //		System.out.println(testCalc.getInfoAboutRupture(194929,
        // erf.getTimeSpan().getStartTimeInMillis()));

        // Biggest diff between RI_TS and RI_NTS on "White Wolf, subsection 0" 2568
        //		System.out.println(testCalc.getInfoAboutRupture(248296,
        // erf.getTimeSpan().getStartTimeInMillis()));

        // NSAF Offshore:
        //		System.out.println(testCalc.getInfoAboutRupture(160395,
        // erf.getTimeSpan().getStartTimeInMillis()));
        ...
        //		System.out.println(testCalc.getInfoAboutRupture(250420,
        // erf.getTimeSpan().getStartTimeInMillis()));
        //
        //		// Biggest gain diff between viable approaches
        //		System.out.println(testCalc.getInfoAboutRupture(240628,
        // erf.getTimeSpan().getStartTimeInMillis()));
