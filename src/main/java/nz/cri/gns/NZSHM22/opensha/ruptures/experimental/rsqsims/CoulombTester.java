...
public class CoulombTester implements Closeable {
    ...
    public CoulombTester(FaultSystemRupSet rupSet, String stiffnessCache, File statsFile)
            throws IOException {
        this.rupSet = rupSet;
    ...
    public List<MultiRuptureJump> getJumps(FaultSystemRupSet rupSet) {
        ...
        return ruptures.getAll().stream()
                .map(ManipulatedClusterRupture::reconstructJump)
    ...
    public void setupStiffness() {
        ...
        MultiRuptureFractCoulombPositiveFilter fractCoulombFilter =
                new MultiRuptureFractCoulombPositiveFilter(
                        stiffness.stiffnessCalc,
                        fractThreshold,
                        ParentCoulombCompatibilityFilter.Directionality.BOTH);
        filters.add(fractCoulombFilter);

        // force the net coulomb from one rupture to the other to positive; this more heavily
        // weights nearby interactions
        ParentCoulombCompatibilityFilter.Directionality netDirectionality =
                ParentCoulombCompatibilityFilter.Directionality
                        .BOTH; // require it to be positive to from subduction to crustal AND from
        // crustal to subduction
        //     	Directionality netDirectionality = Directionality.EITHER; // require it to be
        // positive to from subduction to crustal OR from crustal to subduction
        MultiRuptureNetCoulombPositiveFilter netCoulombFilter =
                new MultiRuptureNetCoulombPositiveFilter(
                        stiffness.stiffnessCalc, netDirectionality);
    ...
    public void writeStats(List<RsqSimEventLoader.Event> events, String fileName)
            throws IOException {
        int ruptureId = 0;
        SelfStiffnessCoulombFilter selfStiffness = new SelfStiffnessCoulombFilter(stiffness);
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
            writer.write(
                    "rupture_id, event_id, FractCoulomb_pass, NetCoulomb_pass,"
                            + selfStiffness.statsHeader());
            writer.newLine();
    ...
    public static void main(String[] args) throws IOException {
        FaultSystemRupSet rupSet1 =
                FaultSystemRupSet.load(
                        new File(
                                "C:\\Users\\user\\GNS\\science-playground\\WORKDIR\\rupSetBruceRundir5883.zip"));
        //  CoulombTester tester = new CoulombTester(rupSet1,
        // "C:\\Users\\user\\GNS\\science-playground\\WORKDIR\\stiffnessCaches");
        ...
        List<List<? extends FaultSection>> sections =
                rupSet1.getFaultSectionDataList().stream()
                        .filter(s -> s.getSectionName().startsWith("Puysegur"))
                        .map(List::of)
                        .collect(Collectors.toList());
