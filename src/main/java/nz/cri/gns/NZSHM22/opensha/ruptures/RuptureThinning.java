...
public class RuptureThinning {
     ...
     */
    public static List<ClusterRupture> filterRuptures(
            List<ClusterRupture> ruptures, Predicate<ClusterRupture> predicate) {
        return ruptures.stream().filter(predicate).collect(Collectors.toList());
    }

    /**
     * Returns a predicate that takes a ClusterRupture and returns true iff the rupture is on a
     * downdip fault.
     *
     * @return
     */
    ...
    /**
     * Returns a predicate that takes a ClusterRupture and returns true iff the rupture's size (in
    ...
    /**
     * Returns a predicate that takes a ClusterRupture and returns true iff the rupture goes from
     * the start of a fault to the end of a fault.
     *
     * @param connectionStrategy the ClusterConnectionStrategy that was used when generating the
     *     ruptures
     * @return the predicate
     */
    public static Predicate<ClusterRupture> endToEndPredicate(
    ...
    static boolean isFaultEnd(
            HashMap<Integer, FaultSubsectionCluster> fullClusters, FaultSection section) {
        FaultSubsectionCluster parent = fullClusters.get(section.getParentSectionId());
        return parent.startSect == section
                || parent.subSects.get(parent.subSects.size() - 1) == section;
    }

    static boolean isEndToEndRupture(
            HashMap<Integer, FaultSubsectionCluster> fullClusters, ClusterRupture rupture) {
        if (fullClusters != null) {
            List<FaultSection> sections = rupture.buildOrderedSectionList();
            return isFaultEnd(fullClusters, sections.get(0))
                    && isFaultEnd(fullClusters, sections.get(sections.size() - 1));
        } else {
            return false;
        }
    }
