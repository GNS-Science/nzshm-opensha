...
public class RsqSimEventLoader {
        ...
        public Map<FaultSection, Double> getSectionFillArea() {
            ...
            for (Patch patch : patches) {
                for (FaultSection section : patch.sections) {
                    result.compute(
                            section,
                            (key, value) -> patch.area + (Objects.isNull(value) ? 0 : value));
                }
            }
            return result;
        }
    ...
    /**
     * Filters input down to single crustal joint ruptures. Side effect: event.jump is populated.
     *
     * @param events
     * @return
     */
    public List<Event> makeSingleJointRuptures(List<Event> events) {
        SectionDistanceAzimuthCalculator disAzCalc =
                new SectionDistanceAzimuthCalculator(
        ...
        List<RsqSimEventLoader.Event> singleCrustalJointRuptures =
                events.stream()
                        ...
                        .filter(event -> event.jump != null) // check if there's a single crustal
                        // rupture
                        .collect(Collectors.toList());
    ...
    public static File findFile(File path, String... candidateNames) throws FileNotFoundException {
        ...
        throw new FileNotFoundException(
                "Could not find candidate files in "
                        + path
                        + " first candidate: "
                        + candidateNames[0]);
    }
    ...
    public List<Event> loadEvents() throws IOException {
        ...
        System.out.println(
                "Loaded " + eList.size() + " rows with " + eList.get(eList.size() - 1) + " events");
    ...
    Map<Integer, Integer> calculateParticipation(List<RsqSimEventLoader.Event> events) {
        ...
        for (RsqSimEventLoader.Event event : events) {
            for (FaultSection section : event.sections) {
                result.compute(
                        section.getSectionId(), (key, value) -> value == null ? 1 : value + 1);
            }
        }
        return result;
    }
     ...
     * @param rupSet
     */
    public void writeParticipationRates(
            List<RsqSimEventLoader.Event> events, FaultSystemRupSet rupSet, String baseOutput) {
