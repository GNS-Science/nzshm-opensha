...
//		}
    ...
    public static ArrayList<GutenbergRichterMagFreqDist> getCharSubSeismoOnFaultMFD_forEachSection(
        ...
        for (int s = 0; s < rupSet.getFaultSectionDataList().size(); s++) {
            ...
            // find the maximum magnitude of the sub seis MFD by getting the index (bin) for the minimum Mag of the section
            int mMaxIndex = totalTargetGR.getClosestXIndex(rupSet.getMinMagForSection(s)) - 1;    // subtract 1 to stay under
...
//			onFaultMFD.set(i,totalTargetGR.getY(i)-offFaultMFD.getY(i));
            ...
            if (offFaultMFD.getY(i) < 0)
                offFaultMFD.set(i, 0); // numerical precision issue at last point if mMaxOffFault = totalTargetGR.getMagUpper()
        }
