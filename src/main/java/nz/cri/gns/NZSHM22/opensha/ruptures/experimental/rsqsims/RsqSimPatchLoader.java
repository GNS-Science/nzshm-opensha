...
public class RsqSimPatchLoader {
    ...
    public static final String RSQSIMS_HIKURANGI = "Hikurangi";
    public static final String RSQSIMS_PUYSEGUR = "Puysegar";
    ...
    public RsqSimPatchLoader(
            File zfaultDeepenIn, PatchesFile patchesFile, File znamesDeepenIn, File rupSet)
    ...
    public void addSectionToPatch(Patch patch, FaultSection section) {
        ...
        patchCountPerSection.compute(
                section, (key, value) -> Objects.isNull(value) ? 1 : value + 1);
    }

    public void findSubductionSections(Patch patch) {
        List<SubductionSection> sections =
                patch.zname.equals(RSQSIMS_HIKURANGI) ? hikurangi : puysegur;
        ...
        patches.forEach(
                p -> {
                    if (p.sectionIdFromZname != -1) {
                        addSectionToPatch(
                                p, loadedRupSet.getFaultSectionData(p.sectionIdFromZname));
                    }
                });
    ...
    /**
     * Writes two CSV files that contain the mappings between patches and sections. outputFile will
     * be a CSV file that has a section id in the first column and matching patch ids in the
     * following columns. outputFile2 has a patch id in the first column and matching section ids in
    ...
    public void writeMappingsToFile(String outputFile, Predicate<FaultSection> sectionFilter)
            throws IOException {
                ...
                .forEach(
                        sectionId -> {
                            ...
                            FeatureProperties props =
                                    patchBuilder.addFaultSectionPolygon(
                                            loadedRupSet.getFaultSectionData(sectionId));
    ...
    public void writeDebugMappings(String baseOutputPath) throws IOException {
        writeMappingsToCsv(
                baseOutputPath + "sectionToPatches.csv", baseOutputPath + "patchToSections.csv");

        // patches debug files
        writeMappingsToFile(
                baseOutputPath + "mappingsCrustal.geojson",
                section ->
                        !section.getSectionName().startsWith("Hikurangi")
                                && !section.getSectionName().startsWith("Hikurangi"));

        writeMappingsToFile(
                baseOutputPath + "mappingsPuysegur.geojson",
                section -> section.getSectionName().startsWith("Puysegur"));

        writeMappingsToFile(
                baseOutputPath + "mappingsHikurangi.geojson",
     ...
     * @throws FactoryException
     */
    public static RsqSimPatchLoader getBrucePatches(String basePath)
        ...
        RsqSimPatchLoader patchLoader =
                new RsqSimPatchLoader(
                        new File(fileName),
                        patchesFile,
                        new File(namesFileName),
                        new File(rupSetFileName));
    ...
    public static RsqSimPatchLoader getCanterburyPatches(String basePath)
            throws IOException, FactoryException {
    ...
    public static void checkSectionEquality(
            List<? extends FaultSection> superSet,
            List<? extends FaultSection> subSet,
            int startIndex) {
    ...
    public static void checkRupSetMatches() throws IOException {
        FaultSystemRupSet rupSetPuy =
                FaultSystemRupSet.load(
                        new File(
                                "C:\\Users\\user\\GNS\\rupture sets\\RupSet_Sub_FM(SBD_0_2_PUY_15)_mnSbS(2)_mnSSPP(2)_mxSSL(0.5)_ddAsRa(2.0,5.0,5)_ddMnFl(0.1)_ddPsCo(0.0)_ddSzCo(0.0)_thFc(0.0)(1).zip"));
        FaultSystemRupSet rupSetHik =
                FaultSystemRupSet.load(
                        new File(
                                "C:\\Users\\user\\GNS\\rupture sets\\RupSet_Sub_FM(SBD_0_3_HKR_LR_30)_mnSbS(2)_mnSSPP(2)_mxSSL(0.5)_ddAsRa(2.0,5.0,5)_ddMnFl(0.1)_ddPsCo(0.0)_ddSzCo(0.0)_thFc(0.0).zip"));
        FaultSystemRupSet rupSetCru =
                FaultSystemRupSet.load(
                        new File(
                                "C:\\Users\\user\\GNS\\rupture sets\\NZSHM22_RuptureSet-UnVwdHVyZUdlbmVyYXRpb25UYXNrOjEwMDAzOA==.zip"));
        FaultSystemRupSet rupSetCombined =
                FaultSystemRupSet.load(
                        new File(
                                "C:\\Users\\user\\GNS\\rupture sets\\nzshm22_complete_merged.zip"));

        Preconditions.checkState(
                rupSetPuy.getNumSections() + rupSetHik.getNumSections() + rupSetCru.getNumSections()
                        == rupSetCombined.getNumSections());
        ...
        checkSectionEquality(
                ...
                rupSetCru.getFaultSectionDataList(),
                cruStart);
        checkSectionEquality(
                rupSetCombined.getFaultSectionDataList(),
                rupSetPuy.getFaultSectionDataList(),
                puyStart);
        checkSectionEquality(
                rupSetCombined.getFaultSectionDataList(),
                rupSetHik.getFaultSectionDataList(),
                hikStart);
    }
