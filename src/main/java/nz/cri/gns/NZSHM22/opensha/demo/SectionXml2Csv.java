...
public class SectionXml2Csv {
    ...
    private static String section2String(FaultSection section) {
        ...
        return new StringBuilder()
                .append(section.getParentSectionId())
                .append(",")
                .append(section.getSectionId())
                .append(",")
                .append(section.getAveDip())
                .append(",")
                .append(section.getAveLowerDepth())
                .append(",")
                .append(section.getAveRake())
                .append(",")
                .append(section.getAseismicSlipFactor())
                .append(",")
                .append(section.getCouplingCoeff())
                .append(",")
                .append(section.getDipDirection())
                .append(",")
                .append(section.getDateOfLastEvent())
                .append(",")
                .append(section.getOrigAveSlipRate())
                .append(",")
                .append(section.getOrigAveUpperDepth())
                .append(",")
                .append(section.getOrigSlipRateStdDev())
                .append(",")
                .append(section.getSlipInLastEvent())
                .append(",")
                .append(locations)
                .toString();
    }
     ...
     * @throws IOException
     */
    private static void printCSVRows(PrintStream out, List<? extends FaultSection> subSections)
            throws IOException {
    ...
    /**
     * Returns a Predicate<FaultSection> that returns true iff the FaultSection is required.
     *
    ...
    private static Predicate<FaultSection> makeSectionFilter(CommandLine cmd) {
        if (cmd.hasOption("ids")) {
            System.err.println(
                    "Only importing IDs " + String.join(", ", cmd.getOptionValues("ids")));

            Set<Integer> filter =
                    Stream.of(cmd.getOptionValues("ids"))
    ...
    private static CommandLine parseCommandLine(String[] args) throws ParseException {
        ...
        Options options =
                new Options()
                        .addRequiredOption("f", "file", true, "an opensha-xml file")
                        .addOption(
                                "l",
                                "maxLength",
                                true,
                                "maxSubSectionLength, will be multiplied with OrigDownDipWidth before use")
                        .addOption("s", "minSections", true, "minSections")
                        .addOption(filterOption)
                        .addOption("o", "out", true, "output file")
                        .addOption("h", "help", false, "prints this help screen");
        ...
        if (null == cmd || cmd.hasOption("help")) {
            ...
            formatter.printHelp(
                    "SectionXml2Csv",
                    "Takes an opensha fault model file and returns a CSV of subsections.",
                    options,
                    "",
                    true);
            System.exit(0);
    ...
    public static void main(String[] args) throws Exception {
        ...
        FaultSectionList sections =
                FaultSectionList.fromList(FaultModels.loadStoredFaultSections(fsdFile));
        ...
        for (FaultSection section : sections) {
            if (filter.test(section)) {
                ...
                List<? extends FaultSection> subSections =
                        section.getSubSectionsList(
                                maxSectLength, allSections.getSafeId(), minSections);
