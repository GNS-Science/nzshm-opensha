...
/**
 * A bunch of helper methods for filtering and combining ruptures. This was originally written for
 * finding suitable rupture combinations for exploring joint rupture algorithms.
 */
public class RuptureQueries {
    ...
    public RuptureQueries(FaultSystemRupSet rupSet) {
        ...
        subductionSections =
                rupSet.getFaultSectionDataList().stream()
                        .filter(RuptureQueries::isSubduction)
                        .collect(Collectors.toList());
        subductionRuptures =
                ruptures.stream().filter(RuptureQueries::isSubduction).collect(Collectors.toList());
        crustalRuptures =
                ruptures.stream().filter(r -> !isSubduction(r)).collect(Collectors.toList());
    }
    ...
    /**
     * Returns true if rupture is a single-cluster subduction rupture. This is sufficient for
     * NZSHM22 ruptures.
     *
     * @param rupture
     * @return
    ...
    /**
     * Build map from subduction section id to crustal ruptures that are within maxJumpDistance of
     * the section.
     *
    ...
    public static Map<Integer, Set<ClusterRupture>> jumpIndex(
        ...
        crustalRuptures.parallelStream()
                .forEach(
                        cr -> {
                            ...
                            for (FaultSection crustSec : crustalSecs) {
                                for (FaultSection subSec : subductionSections) {
                                    if (distAzCalc.getDistance(subSec, crustSec)
                                            <= maxJumpDistance) {
    ...
    /**
     * Take all jumps from a data structure that was created by jumpIndex() This can for example be
     * used to get all crustal ruptures that we can jump to from a subduction rupture.
     *
     * @param sections
     * @param subSecToRups
     * @return
     */
    public static Set<ClusterRupture> jumpTo(
            List<FaultSection> sections, Map<Integer, Set<ClusterRupture>> subSecToRups) {
     ...
     */
    public List<ClusterRupture> homogeniseRuptures(
            List<ClusterRupture> ruptures, float maxAzimuthChange, double maxRakeChange) {
        ...
        TotalAzimuthChangeFilter totAzFilter =
                new TotalAzimuthChangeFilter(azimuthCalc, maxAzimuthChange, true, true);
        result.removeIf(r -> !totAzFilter.apply(r, false).isPass());
        U3CompatibleCumulativeRakeChangeFilter rakeChangeFilter =
                new U3CompatibleCumulativeRakeChangeFilter(maxRakeChange);
        result.removeIf(r -> !rakeChangeFilter.apply(r, false).isPass());
        return result;
    }

    /**
     * Keep all ruptures in the ruptures list that are completely composed of sections that are
     * present in the filter rupture.
     *
     * @param ruptures
     * @param filter
     * @return
     */
    public static List<ClusterRupture> keepIfSubRupture(
            List<ClusterRupture> ruptures, ClusterRupture filter) {
        Set<Integer> ids =
                filter.buildOrderedSectionList().stream()
                        .map(FaultSection::getSectionId)
                        .collect(Collectors.toSet());
        List<ClusterRupture> result = new ArrayList<>(ruptures);
        result.removeIf(
                cr ->
                        cr.buildOrderedSectionList().stream()
                                        .map(FaultSection::getSectionId)
                                        .filter(ids::contains)
                                        .count()
                                != cr.getTotalNumSects());
    ...
    static class RuptureCombination {
        public final ClusterRupture subduction;
        public final List<FaultSection> subductionSections;
        public List<ClusterRupture> crustalRuptures;
        public final ClusterRupture longestCrustal;
        public final double maxLength;
        public long overlap = -1;

        public RuptureCombination(
                ClusterRupture subduction,
                List<FaultSection> sections,
                Collection<ClusterRupture> crustalRuptures,
                Map<Integer, Set<ClusterRupture>> subSecToRups) {
            this.subduction = subduction;
            this.subductionSections = sections;
            this.crustalRuptures =
                    crustalRuptures.stream()
                            .sorted(Comparator.comparing(cr -> cr.buildOrderedSectionList().size()))
                            .collect(Collectors.toList());
            ...
            overlap =
                    subductionSections.stream()
                            .filter(
                                    s -> {
                                        Set<ClusterRupture> jumps =
                                                subSecToRups.get(s.getSectionId());
    ...
    /**
     * Finds suitable candidates for joint rupture algorithm exploration and creates a rupture set
     * and a report.
     *
     * @throws IOException
     */
    public void findCandidates() throws IOException {
        ...
        Map<Integer, Set<ClusterRupture>> subSecToRups =
                jumpIndex(subductionSections, crustalRuptures, distAzCalc, 5);

        Optional<RuptureCombination> candidate =
                this.subductionRuptures.stream()
                        .filter(r -> r.getTotalNumSects() > 8 && r.getTotalNumSects() < 12)
                        .filter(
                                r ->
                                        r.buildOrderedSectionList().stream()
                                                .noneMatch(s -> noWant.contains(s.getSectionId())))
                        .filter(
                                r ->
                                        r.buildOrderedSectionList().stream()
                                                .anyMatch(s -> s.getSectionId() == 2385))
                        .map(
                                r -> {
                        ...
                        .peek(
                                rc ->
                                        rc.crustalRuptures =
                                                keepIfSubRupture(
                                                        rc.crustalRuptures, rc.longestCrustal))
                        .max(
                                Comparator.comparing((RuptureCombination rc) -> rc.overlap)
                                        .thenComparing((RuptureCombination rc) -> rc.maxLength));
        ...
        if (candidate.isEmpty()) {
            ...
            result.crustalRuptures =
                    result.crustalRuptures.stream()
                            .sorted(
                                    Comparator.comparing(
                                                    (ClusterRupture r) ->
                                                            r.clusters[0].startSect.getSectionId())
                                            .thenComparing(ClusterRupture::getTotalNumSects))
                            .collect(Collectors.toList());

            Set<Integer> smallTopRuptureIds = Set.of(2394, 2385); // 2394, 2393, 2384, 2385
            Optional<ClusterRupture> smallTopRupture =
                    this.subductionRuptures.stream()
            ...
            //            System.out.println("candidate size subduction" +
            // result.subduction.getTotalNumSects());
            //            System.out.println("candidate count crustal " +
            // result.crustalRuptures.size());
            //            System.out.println("length " + result.maxLength);
            //            System.out.println("overlap " + result.overlap);
            //            System.out.println("fromStart " + result.crustalRuptures.stream().filter(r
            // -> r.clusters[0].startSect == result.longestCrustal.clusters[0].startSect).count());
            ...
            //                ClusterRupture candidateRupture =
            // MultiClusterRupture.takeSplayJump(result.subduction, rupture);
            //                RupCartoonGenerator.plotRupture(new File(outputDir + "/resources/"),
            // "catalogue" + index, candidateRupture, "catalogue candidate" + index, false, true);
            //                table.add("![entry" + index + "](resources/catalogue" + index +
            // ".png)");
            //                index++;
            //            }
            //
    ...
    public static String writePage(
            String outputDir,
            int subductionIndex,
            ClusterRupture subductionRupture,
            List<ClusterRupture> crustalRuptures,
        ...
        for (ClusterRupture rupture : crustalRuptures) {
            ...
            table.add(
                    "![entry](resources/"
                            ...
                            + index
                            + " stats:  length: "
                            + km.format(rupSet.getLengthForRup(index) / 1000)
                            + " rake: "
                            + df.format(rupSet.getAveRakeForRup(index))
                            + " mag: "
                            + df.format(rupSet.getMagForRup(index)));
            index++;
        }
        ...
        return "[Combinations For Subduction Rupture "
                + subductionIndex
                + "]("
                + htmlFileName
                + ")";
    ...
    public static void main(String[] args) throws IOException {
        RuptureQueries queries =
                new RuptureQueries(
                        FaultSystemRupSet.load(
                                new File(
                                        "C:\\Users\\user\\Dropbox\\RupSet_mergedNZSHM22_Crustal_Hikurangi.zip")));
    ...
    /**
     * A rupture that combines two ruptures. Takes the jump at the start sections of each rupture,
     * assuming that it does not matter.
     */
    public static class MultiClusterRupture extends ClusterRupture {

        protected MultiClusterRupture(
        ...
        public static ClusterRupture takeSplayJump(
                ClusterRupture fromRupture, ClusterRupture toRupture) {
            ImmutableMap.Builder<Jump, ClusterRupture> splayBuilder = ImmutableMap.builder();
            splayBuilder.putAll(fromRupture.splays);
            splayBuilder.put(
                    new Jump(
                            fromRupture.clusters[0].startSect,
                            fromRupture.clusters[0],
                            toRupture.clusters[0].startSect,
                            toRupture.clusters[0],
                            5),
                    toRupture);
