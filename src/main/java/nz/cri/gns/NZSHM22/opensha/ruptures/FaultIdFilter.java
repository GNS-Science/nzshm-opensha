...
public class FaultIdFilter implements PlausibilityFilter {
    ...
    /**
     * Creates a new FaultIdFilter based on the filterType: ANY: Creates a FaultIdFilter that will
     * only accept ruptures that contain at least one of the faults in faultIds. ALL: Creates a
     * FaultIdFilter that will only accept ruptures that contain all of the faults in faultIds.
     * EXACT: Creates a FaultIdFilter that will only accept ruptures that contain exactly the faults
     * in faultIds.
     *
    ...
    public static FaultIdFilter create(FilterType filterType, Set<Integer> faultIds) {
        switch (filterType) {
            case ANY:
                return new FaultIdFilter(
                        ruptureFaults ->
                                intersects(ruptureFaults, faultIds)
                                        ? PlausibilityResult.PASS
                                        : PlausibilityResult.FAIL_FUTURE_POSSIBLE,
                        FilterType.ANY);
            case ALL:
                return new FaultIdFilter(
                        ruptureFaults ->
                                ruptureFaults.containsAll(faultIds)
                                        ? PlausibilityResult.PASS
                                        : PlausibilityResult.FAIL_FUTURE_POSSIBLE,
                        FilterType.ALL);
            case EXACT:
                return new FaultIdFilter(
                        ruptureFaults ->
                                ruptureFaults.equals(faultIds)
                                        ? PlausibilityResult.PASS
                                        : faultIds.containsAll(ruptureFaults)
                                                ? PlausibilityResult.FAIL_FUTURE_POSSIBLE
