...
//import nz.cri.gns.NSHM.opensha.inversion.InversionFaultSystemSolution;
...
/**
 * This class generates background seismicity (gridded) sources for NSHM.NZ
 * 
 * based on scratch.UCERF3.griddedSeismicity.UCERF3_GridSourceGenerator by Ned FIeld and Peter Powers
 * 
 * @author Chris Chamberlain
 */
public class NSHM_GridSourceGenerator extends AbstractGridSourceProvider {
...
//	private double totalMgt5_Rate;
	 ...
	 * 
	 * @param solution {@code InversionFaultSystemSolution} for which
	 *        grided/background sources should be generated
	 */
	public NSHM_GridSourceGenerator(NSHM_InversionFaultSystemSolution solution) {
		/*
		 * 

		branch = solution.getLogicTreeBranch();
		srcSpatialPDF = branch.getValue(SpatialSeisPDF.class).getPDF();
//		totalMgt5_Rate = branch.getValue(TotalMag5Rate.class).getRateMag5();
		realOffFaultMFD = solution.getFinalTrulyOffFaultMFD();
...
//		polyMgr = FaultPolyMgr.create(fss.getFaultSectionDataList(), 12d);
		polyMgr = solution.getRupSet().getInversionTargetMFDs().getGridSeisUtils().getPolyMgr();

		System.out.println("   initSectionMFDs() ...");
		initSectionMFDs(solution);
		System.out.println("   initNodeMFDs() ...");
		initNodeMFDs(solution);
...
//	}
	...
	public static void main(String[] args) {
		...
		//		scaleMFD(grMFD);
		//		System.out.println(grMFD);
		NSHM_InversionFaultSystemSolution invFss = null;
...
//			invFss = (SlipEnabledSolution) FaultSystemIO.loadInvSol(f);
//		} catch (Exception e) {
//			e.printStackTrace();
//		}
		
		NSHM_GridSourceGenerator gridGen = new NSHM_GridSourceGenerator(invFss);
...
//				System.out.println(list.size());
		...
		//		NSHM_GridSourceGenerator gridGen = new UCERF3_GridSourceGenerator(
