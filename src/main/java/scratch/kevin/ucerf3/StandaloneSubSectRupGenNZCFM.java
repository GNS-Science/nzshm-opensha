...
public class StandaloneSubSectRupGenNZCFM {
     ...
     *
     * <p>Based on
     * https://github.com/opensha/opensha-dev/blob/a36d4f49d3fe1a70b86f3ab4b3e2280352a21ecb/src/scratch/kevin/scratch.kevin.ucerf3/StandaloneSubSectRupGen.java
    ...
    public static void main(String[] args) throws DocumentException, IOException {
        ...
        // 83 - 87 Wairarapa
        // 89 - 93 Wellington
        // 94 Wellington Pahiatua
        // 95 - 97 Wellington Tararua

        Stream<Integer> myStream01 = Stream.of(83, 84, 85, 86, 87);
        Collection<Integer> sectsWairarapa =
                myStream01.collect(Collectors.toCollection(ArrayList::new));

        Stream<Integer> myStream02 = Stream.of(89, 90, 91, 92, 93);
        Collection<Integer> sectsWellington =
                myStream02.collect(Collectors.toCollection(ArrayList::new));

        List<Integer> sectsToKeep =
                Lists.newArrayList(Iterables.concat(sectsWairarapa, sectsWellington));

        // List<Integer> sectsToKeep = Lists.newArrayList(16, 90);
        // //Preconditions.checkState(!coulombFilter || fm == FaultModels.FM3_1 || fm ==
        // FaultModels.FM3_2);

        // load in the fault section data ("parent sections")
        List<FaultSection> fsd = FaultModels.loadStoredFaultSections(fsdFile);

        List<Integer> sectsToRemove =
                Lists.newArrayList(
                        17, // Hikurangi
                        27, // Marlborough Slope 4
                        32, // Masterton
                        34, // Mataroa
                        35, // Maunga
                        64, // Rangitikei
        ...
        if (sectsToRemove != null && !sectsToRemove.isEmpty()) {
            System.out.println(
                    "Removing these parent fault sections: " + Joiner.on(",").join(sectsToRemove));
            // iterate backwards as we will be removing from the list
            for (int i = fsd.size(); --i >= 0; )
                if (sectsToRemove.contains(fsd.get(i).getSectionId())) fsd.remove(i);
        }
        //
        ...
        for (FaultSection parentSect : fsd) {
            ...
            List<? extends FaultSection> newSubSects =
                    parentSect.getSubSectionsList(maxSectLength, sectIndex, 2);
        ...
        U3FaultSystemIO.fsDataToXML(
                doc.getRootElement(), FaultModels.XML_ELEMENT_NAME, null, null, subSections);
        ...
        */
        ...
        Map<IDPairing, Double> subSectionDistances =
                DeformationModelFetcher.calculateDistances(maxDistance, subSections);
        ...
        Map<IDPairing, Double> subSectionAzimuths =
                DeformationModelFetcher.getSubSectionAzimuthMap(
        ...
        // laughTest.setMaxJumpDist(2d); 		// looks like this might only impact (parent) section
        // jumps
        // laughTest.setMaxAzimuthChange(Double.MAX_VALUE); // azimuth change constraints makes no
        // sense with 2 axes
        ...
        UCERF3SectionConnectionStrategy connectionStrategy =
                new UCERF3SectionConnectionStrategy(laughTest.getMaxAzimuthChange(), coulombRates);

        // this separates the sub sections into clusters which are all within maxDist of each other
        // and builds ruptures
        // fault model and deformation model here are needed by InversionFaultSystemRuptSet later,
        // just to create a rup set
        // zip file

        // SectionClusterList clusters = new SectionClusterList(
        // 		fm, DeformationModels.GEOLOGIC, laughTest, coulombRates, subSections,
        // subSectionDistances, sectionAzimuths);
        SectionClusterList clusters =
                new SectionClusterList(
                        connectionStrategy,
                        laughTest,
                        subSections,
                        subSectionDistances,
    ...
    private static FaultSection getRemappedSubSect(
            FaultSection origSubSect, List<FaultSection> newSubSects) {
        for (FaultSection newSect : newSubSects) {
            if (newSect.getParentSectionId() != origSubSect.getParentSectionId()) continue;
            if (newSect.getSectionName().equals(origSubSect.getSectionName())) return newSect;
        }
        return null;
    }
}
