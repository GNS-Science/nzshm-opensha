...
- $X^p$: value of $X$ for partition p (e.g. $M_o^s$ is subduction moment)
- $A$: area of rupture
...
Combining equation 1 and 2 and using the seismic moment for only partion $p$:
...
And now we can find $M_w^p$ as a function of the known quantities $A$, $A_c$, and $A_s$ subsituting equation 7 into equation 5
