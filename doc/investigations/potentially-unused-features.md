...
- `NZSHM22_CrustalInversionRunner.setPolygonizer()`
    ...
    - Takes a background pdf grid and re-weights grid points inside fault polygons so that points close to the fault trace Have a lower weight than grid points further away.
...
- `setEnableTvzMFDs()`
    - runzi can set this. nzshm-opensha will currently throw if set to true. should we remove regions?
